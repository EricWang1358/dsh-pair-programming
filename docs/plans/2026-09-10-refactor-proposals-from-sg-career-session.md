...
> | B1 | #8 | ✅ 已修（0.14.1） | B8 | #15 | ⏳ **未做**（开放：债务键去重 + 字节量化测试） |
> | B7 | #9 | ✅ 已修（0.14.1） | C1 | #16 | ✅ 已修（0.14.1） |
> | B5 | #10 | ✅ 已修（0.14.1） | C2 | #17 | ✅ 已修（0.14.1） |
> | B3 | #11 | ✅ 已修（0.14.1） | C3 | #18 | ✅ 已修（0.14.1） |
> | D2 | #20 | ✅ 已实测关闭（结论并入 A3） | D1 | #19 | ⏳ **未实测**（需真实宿主重启，开放） |
> | #21 | 新增 P2 | ⏳ 开放（A1 核对副产：preset 平面写工具绕开全局枚举） | | | |
>
> **发布边界**：package.json / CHANGELOG 已记 0.14.1；**未打 tag、未创建 GitHub Release**——本批指令止于 merge。发布清单（H）里的候选验证与 Release 步骤仍未执行。
>
