...
| K2-6 | **P2：工具级端到端缺的是同一块夹具** | `#75` 的产品语义（只有机长的动作解除暂停）目前只有**调度器级**看守（`wake.test.mjs` 的 `kickTeam` 标志），缺少**真实调用路径**那一层。尝试在 `parallel-tasks.test.mjs` 上补时发现：该套件里**所有** `pair_task_update` 调用都是**被拒的**（`TASK_OWNER` / `INTEGRATION_REQUIRED`），因此没有任何成功路径能让 `flow.js:255` 的 kick 携带标志出现；而构造「成功完成 + 已集成」的并行状态，与 K2-5 缺的是**同一块夹具**（一个可走完的并行任务生命周期）。 | 先补这块夹具（最小：并行队里让一批任务走到 `completed` 且已集成），然后一次性解锁两格：K2-5 的凭证投影 + `#75` 的工具级端到端（成员调 `pair_task_update` 完成 → 断言 kick 的 `background === true`；机长动作 → `false`）。调度器级已捕获的语义不需重测，这格只补调用路径那一层。 |
