...
| 双 Driver | 原生隔离创建 helper 保持两个 cwd、slot 状态已测 | 真实 worktree 分支/HEAD、路由与未集成候选保留；缺树明确拒绝，不自动重建覆盖。**2026-09-11 补**：路线一致性臂已**可执行且门禁**（#41，此前因 harness 缺 `ctx` 而measure的是 harness 自己）；边界回归在已发布树上 **79/0**（parallel-tasks · isolated-members · worktrees · dual-lifecycle · route-isolation）；真 `drivers: 2` 环境的非自造证据仍欠（#26） |
...
| K2-5 | **P2：投影格的仪器缺口** | 想断言 `pair_status` 把陈旧凭证报成 `board_state_current:false` 时，在 `verification.test.mjs` 的合成板面上调用 `pair_status` 会抛 `Cannot read properties of undefined (reading 'length')`；该套件场景助手只记 `e.message` 不记栈，故本轮未能定位。已静态排除：`useCases`（全部 `?? []`）、`productSummary`（`team.product?.discoveries ?? []`）、`yields`/`disclosures`/`gatePasses`（均 `?? []`）。 | 用临时 rethrow-with-stack 或 debugger 定位崩溃点；若确认是合成板面缺 `pair_start` 写入的字段，则补齐种子即可；若确认是产品在缺字段板面上崩溃（老看板！），则按 P1 修 `pair_status` 的健壮性。**规则本身已由 `oracle.test.mjs` 的四个可见性格钉住**，投影只是渲染。 |

| K2-6 | **P2：工具级端到端缺的是同一块夹具** | `#75` 的产品语义（只有机长的动作解除暂停）目前只有**调度器级**看守（`wake.test.mjs` 的 `kickTeam` 标志），缺少**真实调用路径**那一层。尝试在 `parallel-tasks.test.mjs` 上补时发现：该套件里**所有** `pair_task_update` 调用都是**被拒的**（`TASK_OWNER` / `INTEGRATION_REQUIRED`），因此没有任何成功路径能让 `flow.js:255` 的 kick 携带标志出现；而构造「成功完成 + 已集成」的并行状态，与 K2-5 缺的是**同一块夹具**（一个可走完的并行任务生命周期）。 | **2026-09-11 收窄**：`#75` 的调用路径**已无需此夹具**——kick 的成员侧入口其实是 `pair_task_create`（`task.js:190`，普通创建路径；`isCaptain` 只挡 discovery 分支），已在 `backlog.test.mjs` 钉住（成员建卡 → `background === true`；机长建卡 → `false`，8/0/0）。**因此本夹具现在只解锁 K2-5 的凭证投影一格**。 |
...
**落实检查：** 下一次局部修复的测试记录应能逐项解释选测范围，没有理由的慢套件不得进入命令。**2026-09-11 更新**：文档执行规则之外，运行器现已**强制**其中一部分——`--only` 的每个词都必须命中（拼错即 exit 2）、未知参数被拒、摘要回显实际选中集合、`check.skip` 有独立计数且默认致命、**基线外的跳过即使承认也失败**（`tests/skip-baseline.json`）、并行模式报每套件计数与耗时（#65–#71）。**仍属人工判断**：影响面分析（该选哪些套件）与重型套件的拆分——K2-3 已实测证明 `integration` **没有轻量子集可提**，其成本只能靠显式减少默认 scenario 数来降。
...
**2026-09-11 用户裁定：五态止于"已加载"（选项 C）。** 插件**不再**补"已产出/已处理"，因为技能是模型照做的文本而非返回值函数——任何"产出"的说法只能来自**席位自报**（自报证据进 gate 会削弱"只靠可执行产物关闭"这条规矩）或**产物约定**（那要先决定"插件是否包装技能"这个产品语义）。当前落地与其边界：

- **已落地并能机判**：配置/可用（`ce-probe.js` + 车道对模型的暴露面）· 已加载（`ce-loads.jsonl`，且**加载写车道技能会阻断 gate 凭证**）；
- **刻意不做**：已产出/已处理。看板只报它**看得见**的东西（写的是 `Loads`，从不写 `used`）；
- **后果，明确接受**：`ceLanes` 的效果**无法机判**，因此 gate **不得**依赖"审查真的发生过"；
- **重开条件**：出现一次真实的"以为审查发生了、其实没有"的损失（则做席位自报，且**永不挡 gate**），或插件开始**包装**技能（则做产物约定 + 摘要绑定）。
