...
|---|---|---|
| credential projection (K2-5) | `pair_status.gate_credentials[].board_state_current` flips stale after a board move and back after a re-gate | **the reason I first wrote here was wrong.** It is not that `pair_status` throws: a hand-built credential record is **rejected by the store's own state validation** (`readTeam` → "invalid pair-programming state"), so a pass that survives has to be **produced by a real gate run**. The projection therefore needs a fixture that reaches `pair_gate_check` through the protocol, not a seeded one — the rule itself is already pinned in `oracle.test.mjs` **Two more measurements (2026-09-11).** `gateStatePayload` says exactly what moves the digest: the task, its design, its cycles, **P0/P1 risks** (P2 is deliberately excluded) and task-attributed rulings. So the smallest real board move is raising a P1 — and moving the board *through the protocol* is required, because a hand-pushed cycle is rejected by the store just as a hand-built pass is. **But the move has to happen BEFORE completion**: after `pair_task_update` completes the task and the credential is minted, the same attempt hits `GATE_STALE: the latest final review does not bind the current candidate, board and task attempt`, i.e. the review binding is consumed and further state moves through that path are refused. Next attempt therefore: mint the credential, read `board_state_current === true`, move the board **through the protocol while the attempt is still live**, then read the flag. **DECISIVE (2026-09-11, fifth attempt).** Raising a P1 *before* completion raises `GATE_STALE: the task card, cycle record, risk register, or task ruling changed after pair_gate_check`. So the protocol **refuses to move a board that carries a valid credential**, which means `board_state_current === false` is **unreachable through the protocol** — it is a defensive read for boards moved *outside* it (a concurrent writer, a hand edit, an old or damaged board). That is why the earlier attempts felt impossible: they were. The assertion therefore needs an out-of-band write that is **shape-valid** (the store validates on read and write, which is what rejected both a hand-built pass and a hand-pushed cycle) — the concrete next move is to clone an existing element of a real board rather than fabricate one, which keeps validation happy and still moves the digest. |
