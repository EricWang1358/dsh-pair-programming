...
The pattern across three runs is one design fault, not three bugs: **`gateStateFingerprint` answers "did anything on this task's board change" and is used to decide "does the reviewer's judgment about the code still stand".** Everything that happens after a review — another task landing, a phase moving to RETRO, a captain recording a ruling — changes the first and is taken to invalidate the second, while `pair_stop`'s remedy is a call whose own preconditions those same events destroy. What the reviewer actually signed is a candidate and an attempt; what the gate re-checks on its own is the current board, the current tree and the frozen oracle. Binding re-certification to attempt identity and to this task's cycles, and letting the gate's own checklist speak for everything it already re-evaluates, is the repair this evidence points to. It is not made here: it is the third relaxation in a row of an invariant `0a64dc2` deliberately introduced, and it belongs to a deliberate decision rather than to a debugging loop.
