...
- **Settings-UI 卡片已交付（v1.3）**：`lib/client.js` 手写浏览器侧 classic script（`window.__ModuleLoader__.load`，无构建），经 `exports["./client"]` + `dsh.client:{platform:"web"}` 被宿主 client-modules 扫描器发现；卡片注册到 `settings.plugin.item` keyed slot（key = `pair-programming`），7 字段分控件类型（枚举 select / 预算 number / 绿构建 checkbox / DoD text），staged 草稿 + overridden 徽标 + 组合基线展示 + reset + revision-fenced 保存，双语 locale（`settings.pair-programming`）。namespace 未 served 时卡片静默隐藏。
- 实机验证记录：真实 `dsh-settings-file` provider 下 base `off` 被用户节 `coach` 覆盖、describe 正确暴露 user 层；pair-dev headless 真启动零错误；第二 web 实例（禁用 task-board 单例锁的 overlay）实测 client 扫描命中——`__DSH_BOOT__` 图与 `/plugins/??…pair-programming/client.js…` combo script 均出现，bundle 字节经 HTTP 送达验证通过。
