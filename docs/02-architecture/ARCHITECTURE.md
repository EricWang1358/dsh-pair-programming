...
> 默认入口是 solo（SPEC 短命席位），工具共 24 个 `pair_*`（完整清单以 `verify-startup` 为准）。可选双 Driver 见 `README.md` 的 two-contributor workflow。与代码冲突时以代码为准。
>
...
├────────────────────────────────────────────────────────────┤
│ 工具层（注册进共享 tools 注册表；24 个 pair_*，以 verify-startup 为准）│
