...
export async function run(check) {
  ...
  // The green-build rule is no longer honour-system prose: pair_stop executes
  // green_build_command itself and only exit 0 issues a receipt. Pin the
  // parameter name (a captain that does not know it cannot close a board) and
  // the rule, in both modes — not the old hyphenated phrase.
  for (const mode of ['light', 'solo']) {
    const text = captainProtocol({ tddMode: 'enforce', defaultMode: mode }) + usageSectionText({ tddMode: 'enforce', defaultMode: mode });
    check(text.includes('green_build_command'), `personas ${mode} captain is told the parameter pair_stop actually runs`);
    check(/red build|whole-suite green/i.test(text), `personas ${mode} captain green-build rule`);
    check(text.includes('completion_receipt'), `personas ${mode} captain knows the only artefact that closes a goal`);
  }
  ...
  check(usageSectionText({ tddMode: 'enforce', defaultMode: 'light' }).includes('pair_interrupt') && usageSectionText({ tddMode: 'enforce', defaultMode: 'light' }).includes('queued work by default'), 'usage prose names the interrupt hammer and its queue semantics (legacy modes, which have durable seats to interrupt)');
  ...
  check(PROTOCOL_VERSION === '7', 'personas versioned at v7 for isolated contributors and integration');
