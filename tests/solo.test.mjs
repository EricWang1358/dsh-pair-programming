...
export async function run(check) {
  ...
  try {
    ...
    check(solo.includes('max_goal_rounds'), 'B solo names the burn bound — it is a real knob, so it is stated');
    check(solo.includes('refuses pause'), 'B solo states the host refusal — an autonomous pause that cannot work must not be advised');
