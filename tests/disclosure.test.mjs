...
import { declares, knownDisclosureRefs, openDisclosures, residualLedger, residualSummary, unsunkResiduals } from '../lib/protocol/disclosure.js';
...
import { attentionSet } from '../lib/protocol/attention.js';
...
export async function run(check) {
  ...
  try {
    ...
    // V5.2. This exact ruling — "accept visual arm as backlog" — is the measured
    // failure: it read as settled, and the board it deferred to did not exist.
    const undecided = await fails(() => h.tool('pair_arbitrate')({ conflict_ref: 'visual ruling', decision: 'accept visual arm as backlog', evidence: ['review.png'], rationale: 'tracked separately', closes_disclosure: refs[0] }, { agent: h.captain }));
    check(undecided.includes('what happened to the gap') && undecided.includes('fixed|accepted|deferred'), 'closing a disclosure without a disposition is refused');
    const homeless = await fails(() => h.tool('pair_arbitrate')({ conflict_ref: 'visual ruling', decision: 'accept visual arm as backlog', evidence: ['review.png'], rationale: 'tracked separately', closes_disclosure: refs[0], disposition: 'deferred' }, { agent: h.captain }));
    check(homeless.includes('sink') && homeless.includes('residual'), 'a deferred gap with no durable sink is refused — backlog with no backlog entry');
    const badSink = await fails(() => h.tool('pair_arbitrate')({ conflict_ref: 'visual ruling', decision: 'defer', evidence: ['review.png'], rationale: 'r', closes_disclosure: refs[0], disposition: 'accepted', sink: 'transcript', sink_ref: 'the chat' }, { agent: h.captain }));
    check(badSink.includes('unknown sink'), 'only sinks that outlive the session are accepted');
    const ruled = await h.tool('pair_arbitrate')({ conflict_ref: 'visual ruling', decision: 'accept visual arm as backlog', evidence: ['review.png'], rationale: 'the requested structural contract is met; visual polish is tracked separately', closes_disclosure: refs[0], disposition: 'deferred', sink: 'issue', sink_ref: 'GH-412' }, { agent: h.captain });
    check(ruled.closes_disclosure === refs[0], 'pair_arbitrate returns the exact disclosure it closed');
    check(ruled.disposition === 'deferred' && ruled.sink === 'issue' && ruled.sink_ref === 'GH-412', 'the ruling reports the disposition and the durable home of the residual');
    ...
    check(after.protocol.decisions[0].disposition === 'deferred' && after.protocol.decisions[0].sinkRef === 'GH-412', 'the disposition and sink are persisted on the decision record');
    const ledger = residualLedger(after);
    check(ledger.length === 1 && ledger[0].disposition === 'deferred' && ledger[0].sink === 'issue', 'the residual ledger reads back one deferred row with its sink');
    check(residualSummary(after).includes('GH-412'), 'the board summary names where the residual lives');
    check(unsunkResiduals(after).length === 0, 'a ruling recorded through the tool can never be an unsunk residual');
    // A pre-V5.2 record: visible, but not retroactively blocking.
    const legacy = { ...after, protocol: { ...after.protocol, decisions: [{ id: 'd-old', closesDisclosure: refs[0], at: 1 }] } };
    check(unsunkResiduals(legacy).length === 1, 'a legacy ruling with no disposition is surfaced as an unsunk residual');
    check(attentionSet(legacy).items.some(i => i.kind === 'unsunk-residual'), 'the attention set carries the unsunk residual rather than hiding it');
    check(completionReadiness(legacy, { greenEvidence: 'green' }).failures.every(f => !f.includes('unsunk')), 'legacy records are visible but never retroactively block completion');
    ...
    check(status.summary.includes('Residual ledger') && status.residual_ledger.length === 1, 'pair_status renders and returns the residual ledger beside the open disclosures');
    // A fix needs no sink: the gap is gone, and the evidence is the record.
    const fixedRuling = await h.tool('pair_arbitrate')({ conflict_ref: 'beyond ruling', decision: 'removed the extra behaviour', evidence: ['diff'], rationale: 'out of scope', closes_disclosure: refs[1], disposition: 'fixed' }, { agent: h.captain });
