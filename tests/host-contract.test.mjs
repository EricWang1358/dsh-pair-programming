...
export async function run(check) {
  ...
  check(pairTools.length === 28 && ['pair_integrate', 'pair_backlog', 'pair_repair_verify_plan', 'pair_yield', 'pair_cleanup'].every(name => pairTools.some(tool => tool.name === name)), 'all 27 pair_* tools including integration, product backlog, command recovery and the captain yield are accepted by the real registry');
