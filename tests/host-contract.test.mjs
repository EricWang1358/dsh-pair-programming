...
export async function run(check) {
  ...
  check(pairTools.length === 29 && ['pair_integrate', 'pair_backlog', 'pair_repair_verify_plan', 'pair_yield', 'pair_cleanup', 'pair_correction'].every(name => pairTools.some(tool => tool.name === name)), 'all 28 pair_* tools including integration, product backlog, command recovery, the captain yield and the append-only correction are accepted by the real registry');
