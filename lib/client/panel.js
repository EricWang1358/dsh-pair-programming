...
var PAIR_PANEL_ZH={
 ...
 oracle:'验收标准已定',recorded:'已完成',pending:'未完成','check.oracle':'验收标准','check.gate':'质量检查','check.integration':'合并到主线','check.done':'任务收尾',detail:'任务详情',back:'关闭',criteria:'完成标准',design:'设计说明',responsibility:'职责',approach:'做法',failure:'出错时',contract:'约定',
 cyclesHistory:'开发轮次','cycles.empty':'还没有开始开发轮次',repairs:'修正检查命令',history:'最近动态',noHistory:'还没有动态',knownState:'来自团队记录',warning:'部分数据没有读全，页面上没显示的不代表没有进展。',
 ...
 'cycle.PROPOSED':'方案待审','cycle.GO':'方案已通过','cycle.RED':'失败测试已写','cycle.GREEN':'实现与自测已提交','cycle.REFACTOR':'代码已整理','cycle.IMPLEMENTED':'实现已提交','cycle.VERIFIED':'审查已通过','cycle.RISK_CHECKED':'风险已复核','cycle.CLOSED':'轮次已关闭','cycle.accept':'检查通过','cycle.reject':'检查未通过','cycle.checkpoint':'阶段检查通过',
 ...
 rhythm:'工作节奏','rhythm.recentPre':'最近 ','rhythm.recentPost':' 条动态','rhythm.passRate':'判定通过率','rhythm.passRateHint':'只数验证判定：通过、阶段检查通过算通过，未通过算未通过；开工前退回方案不是验证判定，不计入','rhythm.idle':'空闲','rhythm.byTask':'各任务动态','rhythm.other':'其他任务','rhythm.undatedPre':'另有 ','rhythm.undatedPost':' 次未通过没有记录时间',
...
var PAIR_PANEL_EN={
 ...
 oracle:'Acceptance criteria set',recorded:'Done',pending:'Not yet','check.oracle':'Acceptance criteria','check.gate':'Quality check','check.integration':'Merge into main','check.done':'Task close-out',detail:'Task details',back:'Close',criteria:'Done when',design:'Design notes',responsibility:'Responsibility',approach:'Approach',failure:'On failure',contract:'Agreement',
 cyclesHistory:'Work rounds','cycles.empty':'No work round started yet',repairs:'Check command fixed',history:'Recent activity',noHistory:'No activity yet',knownState:'From team records',warning:"Some data couldn't be fully read. Anything missing here isn't necessarily missing progress.",
 ...
 'cycle.PROPOSED':'Approach under review','cycle.GO':'Approach approved','cycle.RED':'Failing test written','cycle.GREEN':'Implementation and self-test submitted','cycle.REFACTOR':'Code tidied','cycle.IMPLEMENTED':'Implementation submitted','cycle.VERIFIED':'Review passed','cycle.RISK_CHECKED':'Risks re-checked','cycle.CLOSED':'Round closed','cycle.accept':'Check passed','cycle.reject':'Check failed','cycle.checkpoint':'Stage check passed',
 ...
 rhythm:'Work rhythm','rhythm.recentPre':'Latest ','rhythm.recentPost':' events','rhythm.passRate':'Verdict pass rate','rhythm.passRateHint':'Counts verification verdicts only: passed and stage-passed count as passes, failed as a failure; a proposal sent back before work starts is not a verdict and is left out','rhythm.idle':'Idle','rhythm.byTask':'Events by task','rhythm.other':'Other tasks','rhythm.undatedPre':'Another ','rhythm.undatedPost':' pushback(s) have no recorded time',
...
function pairRhythm(events,options){
 ...
 return {total:list.length,segments:segments,counts:counts,cycles:cycles,
  passRate:accepted+rejected?accepted/(accepted+rejected):null,verdicts:{passed:accepted,total:accepted+rejected},
  tasks:rows.slice(0,6),other:other.count?other:null};
}
...
// A round's badge is a protocol token (`accept`, `GO`, `GREEN`). Say it in words; an unrecognised
// token still prints, so a protocol that gains a step leaves a readable badge rather than a blank one.
function pairCycleLabel(cycle,t){var token=cycle.verdict||cycle.step||'',key='cycle.'+token;return t(key)!==key?t(key):token;}
...
function createPairDashboard(React){
 ...
 function Rhythm(p){
  ...
  return h('section',{className:'pair-section pair-rhythm'},
   ...
   h('div',{className:'pair-rhythm-stats'},[[t('rhythm.pace'),p.paceMs?pairDuration(p.paceMs,t):'—'],[t('cyclesHistory'),String(data.cycles)],
    [t('rhythm.passRate'),data.passRate===null?'—':Math.round(data.passRate*100)+'%',t('rhythm.passRateHint'),data.passRate===null?null:data.verdicts.passed+' / '+data.verdicts.total]]
    .map(function(s){return h('div',{key:s[0],title:s[2]},h('span',null,s[0]),h('strong',null,s[1],s[3]&&h('small',null,s[3])));})),
 ...
 function Detail(p){
  ...
  return h('section',{ref:root,className:'pair-detail','aria-label':t('detail')},
   ...
   h('div',{className:'pair-checks'},[[t('check.oracle'),a.oracle],[t('check.gate'),a.gate.current],...(p.parallel?[[t('check.integration'),a.integrated]]:[]),[t('check.done'),a.status==='completed']].map(function(x,n){return h('div',{key:x[0],className:x[1]?'is-done':'',style:{'--i':n}},h('i',null),h('span',null,x[0]),h('b',null,t(x[1]?'recorded':'pending')));})),
   ...
   h('h3',null,t('cyclesHistory')),a.cycles.length?h('div',{className:'pair-cycle-list'},a.cycles.map(function(c){return h('div',{key:c.id},h('code',null,c.id),h(Chip,{tone:c.verdict==='reject'?'warn':c.verdict==='accept'?'good':'neutral'},pairCycleLabel(c,t)),c.repairs>0&&h('small',null,t('repairs')+' '+c.repairs));})):h('p',{className:'pair-muted'},t('cycles.empty')));
 }
