...
var PAIR_PANEL_ZH={
 ...
 rhythm:'工作节奏','rhythm.recentPre':'最近 ','rhythm.recentPost':' 条动态','rhythm.passRate':'检查通过率','rhythm.idle':'空闲','rhythm.byTask':'各任务动态','rhythm.other':'其他任务',
...
var PAIR_PANEL_EN={
 ...
 'event.oracle':'Acceptance criteria set','event.review':'Approach reviewed','event.red':'Failing test written','event.green':'Tests passing','event.report':'Implementation submitted',
 ...
 rhythm:'Work rhythm','rhythm.recentPre':'Latest ','rhythm.recentPost':' events','rhythm.passRate':'Check pass rate','rhythm.idle':'Idle','rhythm.byTask':'Events by task','rhythm.other':'Other tasks',
...
function pairRhythmGroup(kind){return kind==='created'||kind==='oracle'?'plan':kind==='reject'?'fail':['accept','checkpoint','integrated','completed'].includes(kind)?'pass':'build';}
...
function pairRhythm(events,options){
 ...
 var counts={plan:0,build:0,pass:0,fail:0},tasks={},cycles=0,accepted=0;
 list.forEach(function(e){
  var g=pairRhythmGroup(e.kind);counts[g]++;if(e.kind==='cycle')cycles++;if(e.kind==='accept'||e.kind==='checkpoint')accepted++;
 ...
 return {total:list.length,segments:segments,counts:counts,cycles:cycles,
  passRate:accepted+counts.fail?accepted/(accepted+counts.fail):null,tasks:rows.slice(0,6),other:other.count?other:null};
}
...
function createPairDashboard(React){
 ...
 function Eta(p){
  var e=p.eta,t=p.t,rough=e.reason==='rough',tick=React.useState(0)[1];
  ...
  return h('span',{className:'pair-eta'+(rough?' is-rough':'')+(left&&left.held?' is-held':''),title:basis},h('i',{className:'pair-eta-mark','aria-hidden':'true'}),
   rough?t('eta')+' '+t('eta.about')+' '+pairDuration(left.ms,t):t('eta.'+e.reason));
 }
 ...
 function Num(p){
  var target=Number(p.value)||0,st=React.useState(function(){return reducedMotion()?target:0;}),shown=st[0],setShown=st[1];
 ...
 return function Dashboard(p){
  ...
  return h('div',{className:rootClass,'data-scheme':p.scheme==='dark'?'dark':'light'},toolbar,
   ...
   h('section',{className:'pair-hero'},h('div',{className:'pair-progress-disc'+(percent>0&&percent<100?' is-partial':''),style:{'--pair-p':percent},title:t('milestoneProgress')+' · '+t('milestoneHint')},h('div',null,h('strong',null,h(Num,{value:percent}),h('small',null,'%')))),
   ...
   progress&&h('section',{className:'pair-section pair-progress-detail'},
    ...
    h('div',{className:'pair-milestones'},progress.milestones.map(function(value,i){return h('div',{key:i,className:c.total&&value>=c.total?'is-done':undefined},h('span',null,t('milestone.'+i)),h('b',null,h(Num,{value:value}),' / '+c.total),h(Bar,{label:t('milestone.'+i),value:value,total:c.total}));}))),
   ...
   h(Swap,{token:p.view,dir:p.viewDir},
    ...
    p.view==='features'&&h(Features,{features:team.features,parallel:team.parallel,paused:p.paused,t:t,openTask:function(id){p.select(id);p.setView('tasks');}}),
    p.view==='activity'&&h('div',{className:'pair-activity'},h(Rhythm,{events:team.history,paceMs:progress?progress.eta.paceMs:null,t:t}),h('div',{className:'pair-activity-log'},history)),
    p.view==='value'&&h(Value,{value:team.value,t:t})),
