  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const receipt = await withLock('pair-integrate-tool:' + stateRoot + ':' + team.id, async () => {
        ...
        if (!candidate) {
          const declared = task.scope?.writes?.length ? task.scope.writes
            : fresh.protocol.cycles.filter(c => c.taskId === task.id).flatMap(c => c.proposal?.files ?? []);
          const snapshot = await snapshotCandidate(slot, [...declared, ...(task.oracle?.files ?? [])]);
        ...
        const promoted = await integrateCandidate(fresh.parallel, candidate, {
          ...
          verify: async workspace => {
            ...
            for (const item of current.tasks.filter(t => t.id === task.id || current.parallel.integrations?.[t.id])) {
              ...
              const run = await runOracleCommand(workspace, item.oracle.cmd, { signal: exec.signal });
