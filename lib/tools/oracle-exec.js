...
/**
 * What an acceptance artifact actually is, in numbers a reader can check against
 * the digest. G4 (r4 session): the freeze message printed one combined digest and
 * nothing else, and a session spent three confusions on it — whether the digest and a
 * file sha256 were the same thing, whether the digest had moved, and whether an
 * unchanged byte count meant unchanged bytes. Beside the seal, print the artifact: per
 * file its sha256, its byte count and its line count.
 *
 * @returns {Promise<{files:{path:string,sha256:string,bytes:number,lines:number}[],bytes:number,lines:number}>}
 */
export async function artifactMetrics(workspace, files) {
  const rows = [];
  let bytes = 0, lines = 0;
  for (const rel of files ?? []) {
    const normalized = String(rel ?? '').replaceAll('\\\\', '/');
    const absolute = resolveInside(workspace, normalized);
    let content;
    try { content = await readFile(absolute); } catch { rows.push({ path: normalized, sha256: '', bytes: 0, lines: 0, missing: true }); continue; }
    const text = content.toString('utf8');
    const newlines = (text.match(/\n/g) ?? []).length;
    const lineCount = newlines + (text.length > 0 && !text.endsWith(String.fromCharCode(10)) ? 1 : 0);
    rows.push({ path: normalized, sha256: createHash('sha256').update(content).digest('hex'), bytes: content.length, lines: lineCount });
    bytes += content.length;
    lines += lineCount;
  }
  return { files: rows, bytes, lines };
}
