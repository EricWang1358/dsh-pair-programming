 ...
 */
export async function checkScope(workspace, declaredFiles) {
  const declared = new Set((declaredFiles ?? []).map(v => String(v).split(String.fromCharCode(92)).join('/').replace(/^[.][/]/, '')));
  let stdout;
  try {
    ({ stdout } = await execFileP('git', ['-C', workspace, 'diff', '--numstat', 'HEAD'], { maxBuffer: 16 * 1024 * 1024 }));
  } catch {
    return undefined;
  }
  const touched = [];
  ...
  return { touched, undeclared, declared: [...declared], netLines: added - removed, added, removed };
}
