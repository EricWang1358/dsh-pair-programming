...
import { stateRootFor, requireAgent, requireParticipantTeam } from './shared.js';
    ...
    async execute(args, exec) {
      ...
      const created = await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        appendPairEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, agent.session), 'pair/task-created', {
          ...
          dependencies: task.dependencies, assignee: task.assignee,
          type: taskType, trivial: task.trivial,
        });
        return { task_id: task.id, assignee: task.assignee, kind: taskType === 'spike' ? 'spike' : (task.trivial ? 'trivial story' : 'story') };
      });
      // Wake idle members so the scheduler can claim the newly-ready work.
      await scheduler.kickTeam(stateRoot, team.id, undefined).catch(() => undefined);
      return created;
    },
  }));
}
