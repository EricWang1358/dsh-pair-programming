...
export function requireLiveEvidenceTarget(team, taskId, agent, { allowCompleted = false } = {}) {
  ...
  const terminal = allowCompleted ? ['failed', 'cancelled'] : ['completed', 'failed', 'cancelled'];
...
export async function captureEvidenceBoundary(team, taskId, agent, config, signal, { allowCompleted = false } = {}) {
  checkCancellation(signal);
  const semantic = semanticStamp(team, taskId, agent, { allowCompleted });
  ...
  return { semantic, worktreeSha, workspace, taskId, allowCompleted };
}
...
export async function assertEvidenceBoundary(boundary, team, agent, config, signal) {
  checkCancellation(signal);
  if (semanticStamp(team, boundary.taskId, agent, { allowCompleted: boundary.allowCompleted === true }) !== boundary.semantic) {
