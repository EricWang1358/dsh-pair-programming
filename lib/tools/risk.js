...
import { openRisk, mitigateRisk, closeRisk, wontfixRisk } from '../protocol/risks.js';
import { isCaptain } from './shared.js';

export function registerRiskTools(ctx, config, runtime) {
  ctx.tools.register(defineTool({
    name: 'pair_risk',
    description: 'Manage one risk ticket. Challenger raises (action=raise, with severity/scenario/trigger/suggestion); Driver mitigates (action=mitigate + evidence); Navigator closes (action=close + evidence); Captain rules a false alarm (action=wontfix + rationale). A P0 raise notifies the captain immediately.',
    parameters: {
