...
export function registerRiskTools(ctx, config, runtime) {
  ctx.tools.register(defineTool({
    name: 'pair_risk',
    description: 'Manage one risk ticket. Raise (action=raise, with severity/scenario/trigger/suggestion); Driver mitigates (action=mitigate + evidence); Navigator closes (action=close); Captain rules a false alarm (action=wontfix + rationale), which is also how a MITIGATED ticket reaches a disposition when no independent artifact exists to close it on. A P0/P1 close needs an EXECUTABLE artifact — closing_cmd, closing_exit=0 and closing_paths — and is refused when every cited artifact is a file this team wrote while working the task: closing a blocker against your own new tests re-asserts the very assumption the ticket doubted. A P2 still closes on a sentence. Until a blocker is CLOSED or WONTFIX it blocks task completion, MITIGATED included. A P0 raise notifies the captain immediately and bypasses the raise budget; a P1/P2 raise is refused once `maxOpenRisks` tickets are already open. Name task_id when the blocker came from one card: it then blocks THAT card, while an unattributed ticket blocks every card.',
    parameters: {
      ...
      scope: { type: 'string', description: 'product (default) | instrument. A product P0 halts new cycles; an INSTRUMENT P0 (a defect in the acceptance oracle, probe or measurement itself) halts verification and the gate instead, so implementation can continue while the measuring device is repaired. Both still block task completion.' },
      ...
      task_id: { type: 'string', description: 'Raise only: the card this blocker came from. An unattributed P0/P1 blocks EVERY card (the default, and what tickets written before this field mean); naming one scopes the block to that card.' },
      ...
      closing_cmd: { type: 'string', description: 'Closing a P0/P1: the command that would FAIL if the risk were real.' },
      closing_exit: { type: 'number', description: 'Closing a P0/P1: that command exit code (must be 0).' },
      closing_paths: { type: 'array', items: { type: 'string' }, description: 'Closing a P0/P1: the artifacts it exercised. At least one must be something this team did not author for this task — the frozen oracle, a pre-existing suite, or an independent probe.' },
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        switch (args.action) {
          case 'raise': {
            ...
            const attributed = args.task_id === undefined ? undefined : String(args.task_id);
            if (attributed !== undefined && !fresh.tasks.some(t => t.id === attributed)) {
              throw new Error(`unknown task "${attributed}" — a risk may only name a card this team has`);
            }
            const risk = openRisk(fresh.protocol, {
              severity: args.severity, scenario: args.scenario, trigger: args.trigger,
              suggestion: args.suggestion, raisedBy: callerRole, scope: args.scope, taskId: attributed,
            });
            result = { risk_id: risk.id, status: risk.status, severity: risk.severity, scope: risk.scope, ...(risk.taskId === undefined ? {} : { task_id: risk.taskId }) };
            break;
          }
          case 'mitigate': {
          ...
          case 'close': {
            ...
            const risk = closeRisk(fresh.protocol, requireArg(args.risk_id, 'risk_id'), requireArg(args.evidence, 'evidence'), {
              closingCmd: args.closing_cmd, closingExit: args.closing_exit, closingPaths: args.closing_paths,
            });
            result = { risk_id: risk.id, status: risk.status };
            break;
          }
