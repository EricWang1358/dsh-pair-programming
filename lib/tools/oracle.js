...
export function registerOracleTools(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const run = await runOracleCommand(workspace, args.oracle_cmd, { signal: exec.signal });
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        for (const cycle of fresh.protocol.cycles) {
          if (cycle.taskId !== task.id || cycle.verify?.verdict !== 'reject' || !['GO', 'PROPOSED'].includes(cycle.step)) continue;
          ...
          cycle.step = 'CLOSED';
          record.supersededRejections.push({ cycleId: cycle.id, ...closure });
        }
