...
import { forkProblems, redProblem, freezeRecord, oracleSummary, reachWarning, nonGatingProblems, boundedRedTail, resealStrengthens } from '../protocol/oracle.js';
...
/**
 * Why the pause below is load-bearing, not a scheduling preference.
 *
 * `.pair-oracles/` sits INSIDE workspaceFingerprint — a new file there is untracked and nothing
 * gitignores it — and pair_gate_check compares that digest against the binding the final ACCEPT
 * captured. Drafting a FUTURE card's oracle file while the current card's candidate is under
 * verification therefore moves the candidate's own binding and stales the gate for work that has
 * not changed. Refuted proposal (#129): "pause only the same task" removes exactly this
 * protection. Measured boundaries: the window never pauses the CURRENT card (writing its own
 * oracle is never blocked), and it only opens once that card has a candidate under verification.
 * The remaining cost — a Navigator idling while another card is verified — is serial-mode
 * throughput, which is a scheduling question with its own answer (`--parallel`/isolated seats),
 * not this guard. Pinned by tests/oracle.test.mjs and tests/verification.test.mjs.
 */
...
export function registerOracleTools(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // The FIRST seal must be shown to fail: an oracle that already passes asserts nothing about a
      // task whose behaviour is missing, and that is the rule's whole point.
      //
      // A RE-FREEZE is different, and this is the cost the rule was charging (#152). Making an arm
      // that was declared non-gating gate again — or adding an arm the implementation already
      // satisfies — means re-sealing a standard whose command now PASSES, and the only way to open a
      // red window was to revert finished code. Forcing a fake red to satisfy a rule that exists to
      // stop fake GREEN is the wrong trade, so a re-freeze may seal green — and it says so, on the
      // record (sealedGreen) and in the receipt. What that costs is local and honest: a green-sealed
      // arm proves the behaviour exists, not that the oracle would catch its absence.
      //
      // Timeouts and unrunnable commands are NOT covered: only exit 0 does.
      //
      // #152 relaxed WHAT a re-freeze must prove, not what a standard may BE. A green re-seal that
      // is WEAKER than the one it replaces — an arm dropped, a command swapped for one that measures
      // less — used to land here and then govern every later verdict, with `sealedGreen` as the only
      // trace and nothing reading it (#162). Refuse it and name the escape.
      const sealingGreen = notRed !== undefined && run?.exit === 0 && task.oracle !== undefined;
      if (sealingGreen && !resealStrengthens(task.oracle, args)) {
        const priorFiles = (task.oracle.files ?? []).map(String);
        const incomingFiles = (args.oracle_files ?? []).map(String);
        const dropped = priorFiles.filter(file => !incomingFiles.includes(file));
        const detail = dropped.length > 0
          ? `it drops oracle file(s) the current seal covers: ${dropped.join(', ')}`
          : `its command is not byte-identical to the current seal's ("${task.oracle.cmd}" -> "${String(args.oracle_cmd ?? '')}")`;
        if (String(args.captain_override ?? '').trim() === '') {
          throw new Error(`task "${args.task_id}" re-freezes an oracle whose command already passes, and the new seal is WEAKER than the one it replaces — ${detail}. GREEN may only be re-sealed when the standard loses nothing it had: the command must be byte-identical (shell composition is not a substitution) and the file set must be a superset. A weaker standard is worse than the red window #152 removed, because it silently governs every later verdict. If the change is deliberate, pass captain_override with a one-line rationale: it seals anyway and the rationale is recorded on the freeze.`);
        }
      }
      if (notRed !== undefined && !sealingGreen) {
      ...
      const record = freezeRecord(args, { sha, run, by: member.name, reach, forks, prior: task.oracle });
      ...
      // Only ever set on a re-freeze, and never on the first seal. A reader who sees it knows the
      // command was already green when this standard was written down: a regression guard.
      if (sealingGreen) record.sealedGreen = { at: record.frozenAt, previousSha: task.oracle.sha ?? null, exit: 0 };
      ...
      return {
        ...
        sealed_green: record.sealedGreen !== undefined,
