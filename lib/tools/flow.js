...
export function registerFlowTools(ctx, config, runtime) {
    ...
    async execute(args, exec) {
      ...
      return withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        if (args.status === 'completed') {
          ...
          if (typeof pass.binding?.worktreeSha !== 'string') {
            throw new Error('GATE_STALE: this gate pass predates worktree binding — run pair_gate_check again');
          }
    ...
    async execute(args, exec) {
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        cycle.proposal = {
          ...
          net_lines: args.net_lines, uncertainty: args.uncertainty ?? '', at: Date.now(),
        };
  ...
  ctx.tools.register(defineTool({
    name: 'pair_verify',
    description: 'Navigator only: close a cycle. On an oracle cycle the verdict is COMPUTED, not asserted — the tool recomputes the frozen digest and re-runs the frozen command, so verdict and evidence arguments are ignored: a passing oracle accepts, a failing one rejects, and an oracle whose bytes moved is an automatic REJECT (an implementation and its acceptance test must never share an author). On a legacy cycle with no oracle you supply the verdict yourself with independently gathered evidence, never parroting the Driver report, and a REJECT needs structured feedback (observation -> impact -> way_forward) plus a reason category.',
    parameters: {
      cycle_id: { type: 'string', required: true },
    ...
    async execute(args, exec) {
      ...
      if (oracle !== undefined) {
        ...
        run = tampered ? undefined : await runOracleCommand(workspace, oracle.cmd);
        computed = computeVerdict({ tampered, run });
      }
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        if (verdict === 'accept') cycle.step = 'VERIFIED';
        else {
      ...
      const sent = await deliverProtocolMessage(ctx, config, agent, team, DRIVER,
        encodeMessage(verdict === 'accept' ? 'ACCEPT' : 'REJECT', {
      ...
      return {
        cycle_id: args.cycle_id, verdict, delivered: sent.delivered, you_owe_next: sent.you_owe_next,
        ...(computed === undefined ? {} : { computed: true, category: computed.category, reason: computed.reason, exit: run?.exit }),
      };
    },
  }));
}
