    ...
    async execute(args, exec) {
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        cycle.corrections = [...(cycle.corrections ?? []), {
          at: Date.now(), by, field, correction: correction.slice(0, 1000), evidence: args.evidence.slice(0, 8),
        }];
        ...
        appendPairEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, agent.session), 'pair/corrected', {
          teamId: fresh.id, cycleId: cycle.id, field, by,
        });
      });
      return { cycle_id: args.cycle_id, field, by, corrections: count };
    },
  }));
