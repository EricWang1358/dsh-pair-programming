...
import { commandShapeError, instrumentExitCodeProblem, normalizeInstrumentExitCodes } from '../protocol/command-shape.js';
import { proposalScopeError } from '../protocol/scope.js';
...
import { isBookkeepingRuling } from '../protocol/disclosure.js';
import { gateStateFingerprint, reviewStateFingerprint, gateBindingDiff } from '../protocol/gate.js';
...
import { claimEligibility, prepareTaskWorkspace, requireTaskOwner, driverForTask, requireIntegratedHead } from '../runtime/parallel-tasks.js';
...
function claimReceipt(team, task) {
  const design = taskDesignContext(team, task);
  return { ...(design.length ? { design_context: design } : {}), task_id: task.id, attempt_id: task.attemptId, attempt: task.attempt ?? 1,
    ...(team.parallel ? { workspace: task.workspace, base_head: task.baseHead,
      write_paths: (task.scope?.writes ?? []).map(path => join(task.workspace, path)),
      read_paths: (task.scope?.reads ?? []).map(path => join(task.workspace, path)) } : {}) };
}
...
/**
 * Append-only record of the pushbacks one cycle received.
 *
 * review/verify are overwritten by the next verdict, so a rejection that was fixed and
 * then accepted survived only as a counter: the panel said "stopped at verification: 3"
 * while its own activity chart showed an empty failed lane — the same page contradicting
 * itself. Evidence is appended, counts stay counts. Bounded like the other per-cycle
 * evidence arrays, oldest first out. The observation is clipped so one long quote cannot
 * grow the board, and it is candidate evidence: exposure.js keeps it off acceptance seats.
 */
const PUSHBACK_LIMIT = 20;
export function logPushback(cycle, entry) {
  const observation = typeof entry.observation === 'string' ? entry.observation.slice(0, 400) : '';
  const record = { kind: entry.kind, stage: entry.stage, ...(entry.category === undefined ? {} : { category: entry.category }), observation, at: entry.at };
  cycle.pushbacks = [...(cycle.pushbacks ?? []), record].slice(-PUSHBACK_LIMIT);
}
...
export function registerFlowTools(ctx, config, runtime) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: `Task ${v.task_id} claimed (attempt ${v.attempt_id}).${v.design_context ? `\nDesign contract: ${JSON.stringify(v.design_context)}` : ''}${v.workspace ? `\nWorking directory: ${v.workspace} (a directory). Declared write paths: ${v.write_paths.join(", ") || "undeclared; inspect task scope"}. Read source files within this directory, not the directory itself. Wait for the task oracle before proposing.` : ''}` }] },
    async execute(args, exec) {
      const agent = requireAgent(exec);
      const stateRoot = stateRootFor(agent, config);
      const team = await requireParticipantTeam(agent, config);
      const claimed = await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        if (owned?.id === task.id && typeof task.attemptId === 'string' && task.attemptId !== '') {
          return claimReceipt(fresh, task);
        }
        if (owned !== undefined) {
      ...
      // Claiming creates the Navigator's oracle obligation in dual mode.
      // Its seat may already be idle after observing the unclaimed board.
      if (team.parallel) await scheduler.kickTeam?.(workspaceOf(agent), team.id).catch(() => undefined);
      return claimed;
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const result = await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        if (args.status === 'completed') {
          ...
          if (gateStateFingerprint(fresh, task.id) !== pass.binding.gateStateSha) {
            // Name what moved. Measured (C1): this refusal listed four candidate
            // inputs, so the captain read the plugin source to find out which one
            // had changed. The credential now carries its own breakdown.
            const changed = gateBindingDiff(fresh, task.id, pass.binding);
            throw new Error('GATE_STALE: the task card, cycle record, risk register, or task ruling changed after pair_gate_check'
              + (changed.length > 0 ? ` — changed since the credential: ${changed.join('; ')}` : '')
              + ' — re-run the gate against the current board');
          }
        ...
        // This refusal has always told the captain to record a ruling first, but the guard only
        // asked whether the card had cycles — so following its own instruction changed nothing.
        // Measured (SG-career): the captain read the message, landed two rulings, was refused
        // again, fell back to `failed` and locked pair_stop into ABORTED. A ruling attributed to
        // THIS card is the recorded reason; one that only discharged a disclosure is the board's
        // paperwork about a gap, not a decision about this card, and does not count.
        const cancelRuling = (fresh.protocol.decisions ?? []).some(d => d.taskId === task.id && !isBookkeepingRuling(d));
        if (args.status === 'cancelled' && specFrozen(fresh.protocol, task.id) && !cancelRuling) {
          throw new Error(`task "${task.id}" already has cycles — cancelling work that started needs a recorded reason: pair_arbitrate({ task_id: "${task.id}", conflict_ref: "cancel ${task.id}", decision, evidence, rationale }) first, then retry with the same attempt_id (marking the task failed stays available for a real outcome)`);
        }
      ...
      if (team.parallel && ['completed', 'failed', 'cancelled'].includes(result.status)) {
        await deliverProtocolMessage(ctx, config, agent, team, 'captain', encodeMessage('INFO', {
          task_id: result.task_id, status: result.status,
          next: 'Read pair_status for remaining work, recovery, or final-tree re-certification and retrospective.',
        }), exec);
        // A member may reach this too: their kick must not lift a captain's pause (#75).
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      instrument_exit_codes: { type: 'array', items: { type: 'number' }, description: 'Exit codes of THIS cycle\'s verify_plan that mean the measurement failed rather than the code (e.g. [2] for a runner that exits 2 when a prerequisite is missing). Declaring one makes that exit produce no checkpoint verdict, no rejection count and a retry hint. 0 is success and anything undeclared stays a verdict about the code.' },
    ...
    async execute(args, exec) {
      ...
      // A declaration defect is not a product verdict either: refuse it here,
      // where the mistake is, rather than at the run.
      ...
      const halting = blocksImplementation(team.protocol, args.task_id);
      if (halting.length > 0) {
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        if (task.ready === false) throw new Error('TASK_NOT_READY: product triage must allocate this draft before implementation');
        const owner = requireTaskOwner(fresh, task, agent);
        const proposalError = proposalScopeError(fresh, task, fresh.protocol.cycles, args.files);
        ...
        if (fresh.parallel && task.status === 'claimed') {
        ...
        cycle.proposal = {
          ...
          ...(cycleWhyNotSplit === undefined ? {} : { whyNotSplit: cycleWhyNotSplit }),
          ...(declaredInstruments.length === 0 ? {} : { instrumentExitCodes: declaredInstruments }),
          at: Date.now(),
        };
        ...
        if (oracle !== undefined) {
          ...
          cycle.red = {
            ...
            evidence: [
              // A green-sealed standard (#152) is a regression guard: it was written down after the
              // behaviour existed, and the record says so instead of claiming a RED nobody ran.
              oracle.redExit === 0
                ? `frozen oracle @${String(oracle.sha).slice(0, 12)} — cmd "${oracle.cmd}" was already GREEN when the standard was sealed (a regression guard, not a RED)`
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: `Cycle ${v.cycle_id}: ${v.verdict.toUpperCase()}${v.conditions_updated ? ' (conditions extended — the step stays GO)' : ''}.` }] },
    async execute(args, exec) {
      const agent = requireAgent(exec);
      const stateRoot = stateRootFor(agent, config);
      const team = await requireParticipantTeam(agent, config);
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        // A second GO on a cycle already at GO is a CONDITION UPDATE, not a step: nothing advances,
        // so "one step at a time" is untouched, and the addition lands on the field the gate and the
        // panel actually read instead of surviving only in a message. Measured (SG-career): the
        // Navigator wanted to add a condition after the Driver had started and had no way to land it
        // (#144).
        const conditionUpdate = args.verdict === 'go' && cycle.step === 'GO';
        const serr = cycleStepError(cycle, args.verdict === 'go' ? 'GO' : 'PROPOSED');
        if (args.verdict === 'go' && serr !== undefined && !conditionUpdate) throw new Error(serr);
        const previousReview = cycle.review;
        const added = String(args.conditions ?? '').trim();
        // Append rather than overwrite: the condition the Driver is already working to must survive
        // the one that was just added (the same rule the pushback record follows).
        const merged = conditionUpdate && added !== '' && typeof previousReview?.conditions === 'string' && previousReview.conditions.trim() !== ''
          ? `${previousReview.conditions}
${added}` : args.conditions;
        cycle.review = {
          verdict: args.verdict, evidence: args.evidence,
          conditions: merged, feedback,
          required_changes: feedback?.way_forward,
          // An update does not re-date the grant: the review happened when it happened.
          at: conditionUpdate ? (previousReview?.at ?? Date.now()) : Date.now(),
          ...(conditionUpdate ? {
            conditionsUpdatedAt: Date.now(),
            conditionLog: [...(Array.isArray(previousReview?.conditionLog) ? previousReview.conditionLog : []), { at: Date.now(), text: added }],
          } : {}),
        };
        conditionsUpdated = conditionUpdate;
        if (args.verdict === 'go') cycle.step = 'GO';
        else {
          cycle.step = 'PROPOSED'; cycle.rejections = (cycle.rejections ?? 0) + 1; fresh.protocol.stats.noGo += 1;
          logPushback(cycle, { kind: 'no_go', stage: 'review', observation: feedback?.observation ?? args.evidence[0], at: cycle.review.at });
        }
        await writeTeam(stateRoot, fresh);
      });
      ...
      return { cycle_id: args.cycle_id, verdict: args.verdict, ...(conditionsUpdated ? { conditions_updated: true } : {}), delivered: sent.delivered, you_owe_next: sent.you_owe_next };
    },
  }));

  /* pair_correction ----------------------------------------------------- */
  // #153. Measured twice on SG-career: a wrong number recorded on an accepted cycle stayed on the
  // board forever, because nothing in the plugin can amend a record (pair_report is refused on an
  // oracle cycle, pair_green is refused once the cycle is settled, and no other path exists).
  //
  // The shape is APPEND-ONLY on purpose: the original stays byte for byte, the correction is another
  // dated, attributed entry beside it. Rewriting is not offered, because the person who wrote the
  // wrong number is the one who would be rewriting it.
  ctx.tools.register(defineTool({
    name: 'pair_correction',
    description: 'Navigator or Captain: append a correction to a cycle record that is wrong — a diff summary, a test result, a claim the evidence does not support. The original stays on the board byte for byte; the correction is a separate dated entry beside it, never a replacement, and the Driver (whose own account it is) may not write one. In solo mode the Captain IS the implementer, so a correction there requires an explicit captain_override and is recorded as a self-correction. The board must still be live (a frozen completion receipt is not a document to be amended) and the caller must belong to the card the cycle is on. Recording a correction rewrites no verdict and invalidates no credential: the field is excluded from the binding digest, because a correction that staled the credential it corrects is the dead end #131 fixed.',
    parameters: {
      cycle_id: { type: 'string', required: true },
      field: { type: 'string', required: true, description: 'Which recorded field is wrong, e.g. "green.test_results" or "report.diff_summary".' },
      correction: { type: 'string', required: true, description: 'What the record should say.' },
      evidence: { type: 'array', items: { type: 'string' }, required: true, description: 'Why the correction is true — command output, file, line.' },
      captain_override: { type: 'string', description: 'Solo mode only: a one-line rationale, required because the Captain is the implementer there and would be correcting its own account. Recorded on the entry as a self-correction.' },
    },
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: `Cycle ${v.cycle_id}: correction for ${v.field} recorded by ${v.by} (${v.corrections} on this cycle). The original is unchanged.` }] },
    async execute(args, exec) {
      const agent = requireAgent(exec);
      const stateRoot = stateRootFor(agent, config);
      const team = await requireParticipantTeam(agent, config);
      const member = team.members.find(m => m.id === agent.id && m.status !== 'removed');
      const by = isCaptain(team, agent) ? 'captain' : member?.role;
      if (by !== 'navigator' && by !== 'captain') {
        throw new Error(`only the Navigator or the Captain corrects a cycle record (you are ${by ?? 'not a member'}) — the Driver's account is the thing being corrected`);
      }
      const override = String(args.captain_override ?? '').trim();
      // The plan's item 2: in solo the Captain IS the implementer and the SPEC seat is refused by the
      // allow-list, so "the author may not correct its own account" has no second writer to appeal to.
      // Refusing outright would leave a wrong number unfixable; allowing it silently would be worse, so
      // the request has to say so and the entry records that it was a self-correction.
      const solo = team.mode === 'solo';
      if (solo && override === '') {
        throw new Error('this team runs in solo mode, where the Captain is also the implementer: the author of the wrong number would be the one correcting it and no other seat exists to write instead. If the record must be corrected anyway, pass captain_override with a one-line rationale — the entry is recorded as a self-correction.');
      }
      const field = String(args.field ?? '').trim();
      const correction = String(args.correction ?? '').trim();
      if (field === '' || correction === '') throw new Error('a correction names the field it corrects and the corrected text');
      requireEvidence(args.evidence, 'a correction');
      let count = 0;
      let byRecorded = by;
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        const fresh = await readTeam(stateRoot, team.id);
        if (fresh === undefined) throw new Error('team no longer exists');
        // The plan's item 3: a correction appended to a dissolved board can rewrite the evidence a frozen
        // receipt cites, so the phase is checked here and the card-level boundary is the same one every
        // other cycle-scoped write uses (which also stops a parallel seat correcting another card).
        if (['DONE', 'ABORTED'].includes(fresh.protocol.phase)) {
          throw new Error(`pair_correction needs a live board; this team is ${fresh.protocol.phase} — a frozen completion receipt is not a document to be amended`);
        }
        const cycle = requireCycle(fresh, args.cycle_id);
        requireLiveEvidenceTarget(fresh, cycle.taskId, agent, { closure: true });
        // Re-derive the seat from the FRESH board: the pre-lock read is a different revision.
        const freshMember = fresh.members.find(m => m.id === agent.id && m.status !== 'removed');
        const byFresh = isCaptain(fresh, agent) ? 'captain' : freshMember?.role;
        cycle.corrections = [...(cycle.corrections ?? []), {
          at: Date.now(), by: byFresh, field, correction: correction.slice(0, 1000), evidence: args.evidence.slice(0, 8),
          ...(override === '' ? {} : { captainOverride: override, selfCorrected: true }),
        }];
        count = cycle.corrections.length;
        await writeTeam(stateRoot, fresh);
        byRecorded = byFresh;
        appendPairEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, agent.session), 'pair/corrected', {
          teamId: fresh.id, cycleId: cycle.id, field, by: byFresh,
        });
      });
      return { cycle_id: args.cycle_id, field, by: byRecorded, corrections: count, ...(override === '' ? {} : { self_corrected: true }) };
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // Asked of THIS cycle's card: a P0 on another card is that card's business (#126).
      const untrusted = blocksVerification(team.protocol, team.protocol.cycles.find(c => c.id === args.cycle_id)?.taskId);
      ...
      if (oracle !== undefined) {
        ...
        if (tampered) {
          ...
          // The declaration travels with the plan: a checkpoint whose command
          // exits 2 because its fixture is missing must not be recorded as
          // checkpoint_red against the Driver's code.
          run = await runOracleCommand(workspace, command, { signal: exec.signal, instrumentExitCodes: normalizeInstrumentExitCodes(target?.proposal?.instrumentExitCodes) });
          ...
          run = await runOracleCommand(workspace, oracle.cmd, { signal: exec.signal, instrumentExitCodes: normalizeInstrumentExitCodes(oracle.instrumentExitCodes) });
          requireProductRun(run, exec.signal);
          computed = computeVerdict({ run, oracle });
        }
      }
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        if (verdict === 'accept' || verdict === 'checkpoint') cycle.step = 'VERIFIED';
          ...
          logPushback(cycle, { kind: 'reject', stage, category: cat, observation: feedback?.observation ?? evidence[0], at: cycle.verify.at });
        }
        taskId = cycle.taskId;
