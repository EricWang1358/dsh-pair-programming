 ...
 * which is now a computed re-run rather than an opinion.
 */
...
function requireRole(team, agent, role, action) {
  ...
  // the checks that used to rest on WHO is speaking now rest on WHAT is
  // computed. A verdict is a re-run of a sealed digest, so it cannot be
  // faked by the same agent that wrote the code — which is the only reason a
  // separate seat was holding the pen in the first place.
  if (team.mode === 'solo') {
    if (member?.role === 'spec') {
...
export function registerFlowTools(ctx, config, runtime) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const sent = await deliverProtocolMessage(ctx, config, agent, team, DRIVER,
  ...
  ctx.tools.register(defineTool({
    name: 'pair_verify',
    description: 'Navigator only: close a cycle with a computed verdict. stage="final" (default) re-runs the full frozen oracle: pass=ACCEPT, fail=REJECT. stage="checkpoint" supports a small intermediate increment: it verifies the frozen oracle digest but runs the proposal\'s predeclared verify_plan instead; pass=CHECKPOINT, which releases the next cycle without claiming the task meets its full acceptance contract. The task gate still requires at least one final ACCEPT and replays the full oracle. A moved oracle is always REJECT. Legacy cycles remain independently asserted with evidence.',
    parameters: {
      cycle_id: { type: 'string', required: true },
      ...
      verdict: { type: 'string', description: 'accept | reject — required on a legacy (no-oracle) cycle; ignored on an oracle cycle.' },
      evidence: { type: 'array', items: { type: 'string' }, description: 'Evidence you gathered independently (legacy cycles only; an oracle cycle records the re-run instead).' },
    ...
    async execute(args, exec) {
      ...
      // N2: an oracle cycle is decided by re-execution, never by a claim.
      ...
      if (oracle !== undefined) {
        ...
        if (tampered) {
          ...
          run = await runOracleCommand(workspace, command);
          computed = run.exit === 0
            ...
            : { verdict: 'reject', category: run.exit === 'timeout' ? 'checkpoint_timeout' : 'checkpoint_red', reason: `the cycle\'s predeclared verify_plan still fails (exit ${String(run.exit)})` };
        } else {
          run = await runOracleCommand(workspace, oracle.cmd);
      ...
      let feedback;
      if (computed === undefined && args.verdict === 'reject') {
        feedback = { observation: args.observation, impact: args.impact, way_forward: args.way_forward };
        const problems = feedbackProblems(feedback);
        if (problems.length > 0) {
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        if (fresh === undefined) throw new Error('team no longer exists');
        const cycle = requireCycle(fresh, args.cycle_id);
        // Computed verdicts (oracle digest + command re-run) certify tree-state,
        // not step position: the step label is a progress pointer with known
        // staleness (serialization mirrors, review not advancing it), and a stale
        // pointer must not veto evidence. Asserted (legacy, no-oracle) verdicts
        // keep the adjacency gate — there the step order is the only record
        // that review happened. A computed verdict still requires a recorded
        // GO review so cycles cannot skip proposal review.
        if (computed === undefined) {
          const serr = cycleStepError(cycle, 'VERIFIED');
          if (serr !== undefined) throw new Error(serr);
        } else if (cycle.review?.verdict !== 'go' && cycle.review?.auto !== true) {
        ...
        cycle.verify = {
          ...
          ...(computed === undefined ? {} : { computed: true, category: computed.category, oracleSha: actualSha, exit: run?.exit }),
          at: Date.now(),
        };
