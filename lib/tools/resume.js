...
export async function requireWorkspaceAvailable(ctx,root,exceptId) {
  const scan=await inspectTeams(root);
  for(const team of scan.teams){
    if(team.id===exceptId||terminal(team))continue;
    if([team.captainSessionId,...team.members.filter(m=>m.status!=='removed').map(m=>m.id)].some(id=>ctx.agents.get(id)!==undefined))
      throw new Error('PAIR_WORKSPACE_BUSY: team '+team.id+' still owns this checkout. Use a separate project working directory for an unrelated concurrent goal; artifact namespaces do not isolate production files.');
  }
}
...
export async function resumeTeam(ctx,config,captain,args,runtime,signal) {
  ...
  return withLock('workspace-start:'+root,()=>withLock(teamLockKey(root,id),async()=>{
    ...
    if (await findTeamByCaptain(root,captain.id)) throw new Error('The new Captain already leads a live team');
    ...
    try {
      for (let i=0;i<team.members.length;i++) {
        ...
        const selection=await resolveMemberLlmSelection(ctx,captain,seatModelRequest(config,prior.role),signal);
