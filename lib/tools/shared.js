...
import { prepareMailboxDelivery, finishMailboxDelivery, coalesceDelivery, deliveryKey,
  urgentMail, busyNotificationAllowed, markBusyNotification } from '../runtime/mail-delivery.js';
...
export async function deliverProtocolMessage(ctx, config, caller, team, to, content, exec) {
  ...
  let refusal;
  let batch;
  const key = deliveryKey(stateRoot, team.id, to);
  await coalesceDelivery(ctx, key, async () => {
    ...
    if (recipient === undefined || recipient.id === '') return;
    const live = to === CAPTAIN_KEY ? captain : ctx.agents.get(recipient.id);
    const busy = live !== undefined && live.status !== 'idle';
    if (!busyNotificationAllowed(ctx, key, recipient.id, busy) || (busy && !urgentMail(message))) return;
    ...
    if (accepted) {
      if (busy) markBusyNotification(ctx, key, recipient.id);
  ...
  return {
    ...
    ...(batch === undefined ? {} : { delivery_batch: { selected: batch.count, bytes: batch.bytes, backlog: batch.backlog } }),
