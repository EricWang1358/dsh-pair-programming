...
import { resolveMemberLlmSelection, spawnMember, interruptMember, markMemberRetired } from '../runtime/members.js';
...
export async function retireSpawnedMembers(ctx, captain, stateRoot, members) {
  ...
  for (const member of spawned) {
    ...
    try {
      // Public Agent.cancel clears both next-turn and next-step queues. The
      // subagent interrupt remains the ancestry-authorized fallback.
      live?.cancel?.({ kind: 'disposed' });
    } catch (error) {
      ctx.logger.warn(`pair-programming: clearing retired member ${member.id} failed: ${String(error)}`);
    }
...
export function registerLifecycleTools(ctx, config, runtime) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      use_cases: { type: 'array', items: { type: 'object', additionalProperties: true }, required: true, description: 'Every independently observable requirement, before implementation: [{actor,intent,outcome,acceptance_criteria[]}]. The plugin assigns UC-N / UC-N.AC-N ids and requires every criterion on a task, in an oracle, and completed.' },
    ...
    async execute(args, exec) {
      ...
      const mode = args.mode === undefined ? configuredMode : args.mode;
      ...
      return withLock(captainLockKey(stateRoot, captain.id), async () => {
        ...
        return withLock(teamLockKey(stateRoot, teamId), async () => {
          ...
          const roles = mode === 'solo' ? ['spec']
            : mode === 'light' ? ['driver', 'navigator']
              : ['driver', 'navigator', 'challenger'];
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      const summary = [
        ...
        `Tuned to the instrument: ${p.cycles.filter(c => (c.green?.tunedForOracle ?? 'none').toLowerCase() !== 'none').map(c => `${c.id}: ${c.green.tunedForOracle}`).join(' · ') || 'nothing declared'}`,
  ...
  ctx.tools.register(defineTool({
    name: 'pair_stop',
    description: 'Close a team with an explicit outcome. outcome="complete" is a hard terminal gate: every goal criterion must be allocated, executable, and completed; every task must carry its exact gate pass; no P0/P1 remains; RETRO is done; and the plugin itself runs green_build_command (or configured dodCommand) fresh and observes exit 0. It returns a durable completion_receipt for the host goal. outcome="aborted" records an honest non-success terminal state and never issues a receipt.',
    parameters: {
      ...
      green_build_command: { type: 'string', description: 'Whole-suite command the plugin must execute fresh. Required for successful stop when no dodCommand is configured.' },
    ...
    async execute(args, exec) {
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        if (outcome === 'aborted') {
          ...
          const readiness = completionReadiness(fresh, { greenRequired: config.greenBuildOnStop !== false, greenEvidence: evidence });
          if (!readiness.ready) {
