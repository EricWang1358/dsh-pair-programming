...
import { createDriverWorktrees, removeDriverWorktrees } from '../runtime/worktrees.js';
...
export function registerLifecycleTools(ctx, config, runtime) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      drivers: { type: 'number', description: '1 (default) or 2. Two Drivers require a clean Git repository and use separate worktrees with serialized, tested integration. Omitted mode selects light when drivers=2.' },
      integration_command: { type: 'string', description: 'Required for drivers=2 unless dodCommand is configured: whole-suite regression command run on every merged candidate.' },
    ...
    output: {
      ...
      render: (_args, value) => [{ type: 'text', text: `Pair team "${value.team_id}" formed (${value.mode} mode), members: ${value.members.join(', ')}. Phase: PLANNING.${value.parallel_policy ? `\n${value.parallel_policy}\nWorkspaces: ${JSON.stringify(value.workspaces)}` : ''}` }],
    },
    async execute(args, exec) {
      ...
      const integrationCommand = String(args.integration_command ?? config.dodCommand ?? '').trim();
      if (drivers === 2) {
