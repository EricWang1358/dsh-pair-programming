...
export function registerLifecycleTools(ctx, config, runtime) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      const effectiveScope = team.tasks.map(task => {
        ...
        return { task_id: task.id, source, envelope, writes: allowed };
      });
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: (v.delivered
        ...
        ? `Cancel signal issued for ${v.interrupted} (${v.reason}); this run will not be woken again until you speak. The host is fire-and-return, so if it was inside a long step it may still be finishing it — check pair_status.`
