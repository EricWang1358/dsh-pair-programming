...
export function registerLifecycleTools(ctx, config, runtime) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      const summaryLines = [
        ...
        ...(team.product ? [`Product backlog: ${product.untriaged} awaiting Captain triage; ${product.deferred} deferred; showing ${product.discoveries.length}/${product.total}, offset ${product.offset}${product.next_offset === null ? "" : `; next pair_status(product_offset=${product.next_offset})`}.`, ...product.discoveries.map(item => exposeCandidate
            ? `${item.id} [${item.status}${item.priority ? `/priority ${item.priority}` : ''}]: ${item.observation} — ${item.userValue}`
            : `${item.id} [${item.status}${item.priority ? `/priority ${item.priority}` : ''}] (text withheld — candidate evidence)`)] : []),
        ...
        // `noOracleReason` is in CANDIDATE_CYCLE_FIELDS because the Driver writes it, so the object
        // projection strips it — and this summary line put it straight back (the plan's item 4).
        `Oracle bypasses: ${p.cycles.filter(c => c.noOracleReason).map(c => exposeCandidate ? `${c.id}: ${c.noOracleReason}` : `${c.id} (reason withheld — candidate evidence)`).join(' · ') || 'none'}`,
        ...
        `Tuned to the instrument: ${p.cycles.filter(c => declares(c.green?.tunedForOracle)).map(c => exposeCandidate ? `${c.id}: ${c.green.tunedForOracle}` : `${c.id} (text withheld — candidate evidence)`).join(' · ') || 'nothing declared'}`,
        ...
        `Open disclosures: ${disclosureSummary(team, { quoted: exposeCandidate })} — each needs a captain ruling before successful stop`,
      ...
      return {
        ...
        // The plan's item 6: a risk's scenario/suggestion and a discovery's observation are free text
        // the Driver can write (pair_risk is inside the Driver's allow-list), so a seat that must not
        // read the candidate keeps the board's shape — id, severity, scope, state — and loses the wording.
        product: exposeCandidate ? product : {
          ...product,
          discoveries: (product.discoveries ?? []).map(({ id, status }) => ({ id, status })),
        },
        ...
        risks: exposeCandidate ? p.risks : p.risks.map(({ id, severity, scope, status }) => ({ id, severity, scope, status })),
        open_risks: exposeCandidate ? openRisks : openRisks.map(({ id, severity, scope, status }) => ({ id, severity, scope, status })),
  ...
  ctx.tools.register(defineTool({
    name: 'pair_interrupt',
    description: 'Captain only: ISSUE a cancel signal for one member turn, then stop the sweep from reviving that run until you speak again. The host is fire-and-return — "the target may keep running until it observes the signal" (dsh-subagent types/index.d.ts:146-152) — so a member inside a long tool call can finish that call first; check pair_status before assuming it stopped. This is a PAUSE, not a terminal stop. The pause removes the run from the heartbeat sweep and keeps it there: every automatic edge carries a background flag, so a member finishing a turn, a peer mailbox recovery kick, or a heartbeat pass already in flight does not lift it — at most ONE further nudge from a pass that was already in flight can slip through. A peer protocol message to a member is still written to its durable mailbox, but is not live-woken while the pause holds; it is handed over on the next kick after the pause lifts. The pause is lifted by a deliberate captain action — a pair_* call you make that kicks the team (`pair_task_create`, `pair_arbitrate`, a verdict you send), pair_resume, or a host restart — never by a member seat going idle. By default this also clears that child\'s host inbox and acknowledges its unread pair mailbox, so stale instructions cannot flood the next turn. Pass discard_queued=false only when the queued work is still valid.',
    parameters: {
    ...
    async execute(args, exec) {
      ...
      await withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        // no board state and no new field: a deliberate captain action re-tracks the run (a
        // member idle edge and a peer recovery kick carry the background flag), so the pause
        // lasts until the captain speaks again.
        scheduler?.untrackTeam?.(workspaceOf(captain), fresh.id);
