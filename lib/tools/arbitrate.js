...
import { knownDisclosureRefs, dispositionError } from '../protocol/disclosure.js';
...
import { gateStateFingerprint, reviewStateFingerprint, runGate, isSupersededRejectedCycle } from '../protocol/gate.js';
...
description: 'Captain only: resolve a conflict (Driver/Navigator disagreement, a contested risk, or an approach decision) on repository evidence. A ruling attributable to one task spends that task\'s planning budget (planningMaxArbitrations); a task that already has cycles is exempt, and a ruling that names no task falls back to the currently claimed task budget rather than costing nothing (the loophole that let a captain write ~18 rulings against a 2/task cap by omitting task ids), and record the decision with rationale. Apply the 70% rule: for reversible decisions, strong (not perfect) evidence is the bar — waiting for certainty costs more than a cheap correction. Notifies the whole team.',
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const boundary = await captureEvidenceBoundary(team, args.task_id, agent, config, exec.signal);
      ...
      if (latestReview?.verdict === 'accept') {
        ...
        if (binding?.worktreeSha !== boundary.worktreeSha || binding?.gateStateSha !== reviewStateFingerprint(team, args.task_id)
          || binding?.assignee !== (reviewedTask.assignee ?? null) || binding?.attemptId !== (reviewedTask.attemptId ?? null) || binding?.handoffId !== (reviewedTask.handoffId ?? null)) {
          throw new Error('GATE_STALE: the latest final review does not bind the current candidate, board and task attempt (or predates review binding). Preserve the old verdict; open a fresh cycle, report GREEN and obtain a new final review before gating.');
        }
      }
      ...
      const inheritedFiles = team.protocol.cycles
        ...
        .flatMap(c => c.proposal?.files ?? []);
      const scopeCheck = await checkScope(workspace, declaredFiles, team.baseline, inheritedFiles);
      const oracle = gatedTask?.oracle;
      let oracleReplay;
      // Only explicitly superseded rejected cycles may retain an old seal.
      // A checkpoint or arbitrary CLOSED record does not erase the mismatch.
      const strayCycle = team.protocol.cycles
        .filter(c => c.taskId === args.task_id && !isSupersededRejectedCycle(c, oracle))
      ...
      if (strayCycle !== undefined) {
        ...
        if (actualSha !== oracle.sha) {
          ...
          const run = await runOracleCommand(workspace, oracle.cmd, { signal: exec.signal });
      ...
      return withLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        const pass = recordGatePass(fresh, args.task_id, outcome.checklist, {
          ...
          taskAttempt: { assignee: targetTask.assignee ?? null, attemptId: targetTask.attemptId ?? null, handoffId: targetTask.handoffId ?? null },
