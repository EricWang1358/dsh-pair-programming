...
export function registerArbitrateTools(ctx, config, runtime) {
  ...
  ctx.tools.register(defineTool({
    name: 'pair_arbitrate',
    description: 'Captain only: resolve a conflict (Driver/Navigator disagreement, a contested risk, or an approach decision) on repository evidence, and record the decision with rationale. Apply the 70% rule: for reversible decisions, strong (not perfect) evidence is the bar — waiting for certainty costs more than a cheap correction. Notifies the whole team.',
    parameters: {
