...
export function apply(ctx, config) {
  const resolved = {
    stateDir: config.stateDir ?? '.pair-programming',
    memberProvider: config.memberProvider ?? 'spawn',
    memberModel: config.memberModel,
    memberMaxDepth: config.memberMaxDepth ?? 1,
    maxMembers: config.maxMembers ?? 4,
    maxCyclesPerTask: config.maxCyclesPerTask ?? 12,
    defaultMode: config.defaultMode ?? 'full',
    tddMode: config.tddMode ?? 'enforce',
    pairStyle: config.pairStyle ?? 'traditional',
    spikeMaxCycles: config.spikeMaxCycles ?? 2,
    greenBuildOnStop: config.greenBuildOnStop ?? true,
    dod: String(config.dod ?? '').split(',').map(s => s.trim()).filter(Boolean).length > 0
      ? String(config.dod).split(',').map(s => s.trim()).filter(Boolean)
      : undefined, // undefined -> the gate runs its DEFAULT_DOD list
    evidenceCache: config.evidenceCache ?? true,
    promptSectionOrder: config.promptSectionOrder ?? 118,
    slashCommand: config.slashCommand ?? true,
  };

  // 1. Member model-selection bridge (fresh + cold-resume), riding
  //    registerContinuableSetup. Provider capability validation happens at the
  //    first member spawn, once the sibling provider row has settled.
