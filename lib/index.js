...
import { configureIsolatedMembers } from './runtime/isolated-members.js';
...
import { registerIntegrationTools } from './tools/integrate.js';
import { registerBacklogTools } from './tools/backlog.js';
import { registerYieldTools } from './tools/yield.js';
import { registerCleanupTools } from './tools/cleanup.js';
import { installPairPanel } from './runtime/panel-rpc.js';
...
export function apply(ctx, config) {
  ...
  configureIsolatedMembers(ctx, resolved);
  ...
  registerMailboxTools(ctx, resolved);
  registerIntegrationTools(ctx, resolved, runtime);
  registerBacklogTools(ctx, resolved, runtime);
  registerYieldTools(ctx, resolved, runtime);
  registerRepairTools(ctx, resolved, runtime);
registerCleanupTools(ctx, resolved, runtime);

  // 4. The captain's usage policy in the global system prompt.
  ctx.systemPrompt.section({
    name: 'pair-programming:usage',
    order: resolved.promptSectionOrder,
    text: usageSectionText({ tddMode: resolved.tddMode, pairStyle: resolved.pairStyle, defaultMode: resolved.defaultMode, experimentalLeanPrompt: resolved.experimentalLeanPrompt === true, experimentalDualDrivers: resolved.experimentalDualDrivers, dualDriverIntegrationCommand: resolved.dualDriverIntegrationCommand, ceLanes: resolved.ceLanes, ceSoloLane: resolved.ceSoloLane }),
  });
