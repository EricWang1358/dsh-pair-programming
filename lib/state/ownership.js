...
export async function workspaceOwner(stateRoot, exceptId) {
  ...
  // inspectTeams reports a PARTIAL scan through { errors, complete }, not by throwing.
  // Ignoring that was the same fail-open the try/catch above already refuses: the board
  // that could not be read may be the very team that owns this checkout, so unknown
  // must refuse rather than license a second writer.
  if (scan.complete !== true) {
    const broken = (scan.errors ?? []).map(entry => String(entry?.teamId ?? 'unknown')).join(', ');
    return { teamId: '(unreadable board: ' + (broken === '' ? 'unknown' : broken) + ')', phase: 'unknown' };
  }
