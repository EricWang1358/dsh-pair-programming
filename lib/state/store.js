...
/** Compare the original seat identity under the team lock before committing.
 * A child id is the generation token; no new on-disk schema is required.
 * commit is injectable for deterministic storage-failure tests.
 */
export async function commitMemberReplacement(stateRoot, teamId, original, replacement, commit = writeTeam) {
  return withLock(teamLockKey(stateRoot, teamId), async () => {
    const fresh = await readTeam(stateRoot, teamId);
    if (fresh === undefined || isDispatchClosed(fresh.protocol.phase)) return false;
    const target = fresh.members.find(m => m.name === original.name && m.status !== 'removed');
    if (target === undefined || target.id !== original.id || target.joinedAt !== original.joinedAt || target.role !== original.role) return false;
    const history = Array.isArray(target.seatHistory) ? target.seatHistory : [];
    target.seatHistory = [...history.slice(-11), {
      at: replacement.joinedAt, reason: replacement.replacementReason === 'accepted-cycle' ? 'accepted-cycle' : 'recovery',
      previousId: target.id, nextId: replacement.id,
    }];
    target.replacementCount = (Number.isSafeInteger(target.replacementCount) && target.replacementCount >= 0 ? target.replacementCount : 0) + 1;
    Object.assign(target, {
      id: replacement.id, status: 'idle', joinedAt: replacement.joinedAt,
      provider: replacement.provider, model: replacement.model, reasoningEffort: replacement.reasoningEffort,
    });
    if (replacement.runtime === 'isolated') {
      target.runtime = replacement.runtime;
      target.workspace = replacement.workspace;
      target.composition = replacement.composition;
    }
    delete target.activity; delete target.lastTurn; delete target.resume; delete target.obligationDelivered;
    await commit(stateRoot, fresh);
    return true;
  });
}
...
export async function findTeamByCaptain(stateRoot, captainSessionId) {
  ...
  for (const teamId of entries) {
    ...
    if (team?.captainSessionId === captainSessionId) {
      if (team.protocol.phase === 'DONE' || team.protocol.phase === 'ABORTED') continue; // terminal teams are archives
...
export async function findTeamByParticipant(stateRoot, agentSessionId) {
  ...
  for (const teamId of entries) {
    ...
    if (participates && team !== undefined) {
      if (team.protocol.phase === 'DONE' || team.protocol.phase === 'ABORTED') {
...
/**
 * Whether any non-terminal team exists under this state root.
 *
 * Workspace-scoped rather than session-scoped on purpose: the CE lane resolver
 * runs inside a skill-provider call, which is given a cwd and no agent
 * identity. That is the same attribution limit the load ledger has, and it errs
 * the safe way — while ANY team is live in the workspace, every session there
 * gets the narrow pair lane.
 */
export async function hasActiveTeam(stateRoot) {
  for (const teamId of await listTeamDirs(stateRoot)) {
    const team = await readTeam(stateRoot, teamId);
    const phase = team?.protocol?.phase;
    if (phase !== undefined && phase !== 'DONE' && phase !== 'ABORTED') return true;
  }
  return false;
}

export async function listTeamDirs(stateRoot) {
  let entries;
  try {
  ...
  return entries.filter(e => e.isDirectory() && e.name !== 'cache').map(e => e.name).sort();
}

/** Recovery discovery only. Authorization callers still use strict reads. */
export async function inspectTeams(stateRoot) {
  const teams = [], errors = [];
  for (const teamId of await listTeamDirs(stateRoot)) {
    try { const team = await readTeam(stateRoot, teamId); if (team !== undefined) teams.push(team); }
    catch (error) { errors.push({ teamId, error: String(error) }); }
  }
  return { teams, errors, complete: errors.length === 0 };
}
...
export function recordGatePass(team, taskId, checklist, binding = undefined) {
  const { at: _at, ...stableChecklist } = checklist ?? {};
  const signature = createHash('sha256').update(JSON.stringify({ taskId, checklist: stableChecklist, binding })).digest('hex');
  const previous = latestGatePass(team, taskId);
  // Replaying the same gate over the same files and board facts returns the
  // same credential. Random ids on byte-identical replays made the board show
  // several plausible "final" ids and forced callers to trust seat narration.
...
function isTeamTask(value) {
  return isRecord(value)
    ...
    && isOptionalString(value['gatePassId'])
    && (value['acceptanceRefs'] === undefined || (Array.isArray(value['acceptanceRefs']) && value['acceptanceRefs'].every(v => typeof v === 'string')))
...
function isTeamState(value, expectedId) {
  ...
  return value['id'] === expectedId
    ...
    && (value['mode'] === 'solo' || value['mode'] === 'light' || value['mode'] === 'full')
    ...
    && (value['useCases'] === undefined || Array.isArray(value['useCases']))
