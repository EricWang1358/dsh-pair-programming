...
/** Host acceptance ACK is NOT proof of model reading or completion. */
function eligible(message, now = Date.now()) {
  return message.readAt === undefined && (message.deliveryClaimedAt === undefined
    || now - message.deliveryClaimedAt >= MAILBOX_DELIVERY_LEASE_MS);
}

/** Read messages eligible for host notification (legacy field name: readAt). */
export async function readUnreadMailbox(stateRoot, teamId, agentKey, onMalformedLine) {
  ...
  return (await readMailbox(stateRoot, teamId, agentKey, onMalformedLine))
    .filter(message => eligible(message, now));
}
...
export async function claimMailboxDelivery(stateRoot, teamId, agentKey, messageIds) {
  const now = Date.now();
  const claimId = randomUUID(), claimed = [];
  await mutateMailbox(stateRoot, teamId, agentKey, messageIds, message => {
    if (!eligible(message, now)) return message;
    claimed.push(message.id);
    return { ...message, deliveryClaimedAt: now, deliveryClaimId: claimId };
  });
  return { claimId, messageIds: claimed };
}

/** Release a failed delivery lease so the scheduler can retry it later. */
export async function releaseMailboxDelivery(stateRoot, teamId, agentKey, messageIds, claimId) {
  await mutateMailbox(stateRoot, teamId, agentKey, messageIds, (message) => {
    if (claimId !== undefined && message.deliveryClaimId !== claimId) return message;
    const { deliveryClaimedAt: _claimed, deliveryClaimId: _token, ...released } = message;
    return released;
  });
}

/**
 * Record host acceptance, NOT model reading/completion. readAt is retained for
 * existing readers; the durable content stays available through pair_mailbox_read.
 * Tokenless calls retain legacy explicit-discard semantics. Delivery callers
 * must supply their lease token. Callers serialize this with the team lock.
 */
export async function acknowledgeMailbox(stateRoot, teamId, agentKey, messageIds, claimId) {
  const now = Date.now();
  await mutateMailbox(stateRoot, teamId, agentKey, messageIds, (message) => {
    if (claimId !== undefined && message.deliveryClaimId !== claimId) return message;
    const { deliveryClaimedAt: _claimed, deliveryClaimId: _token, ...rest } = message;
    return {
      ...rest,
