...
export function buildActivationDirective(goal) {
  ...
  lines.push('If two Drivers are requested but the workspace lacks a Git repository or committed checkpoint, propose preserving the current checkpoint as a local Git baseline. Explain the file scope and ignore rules, ask for explicit user approval before initializing or committing, then retry pair_start. Offer one Driver if declined. Existing teams must preserve their board, evidence and handoff; never bypass unfinished gates or abort solely to change mode.');
  lines.push('For a requested continuation, inspect pair_status(list_runs=true), then explicitly resume the matching run with pair_start(resume_team, resume_from_captain), omitting new-team fields. Keep its existing board and evidence. Sharing a workspace never means sharing a project: unrelated goals start a fresh run and do not inherit prior lessons by default.');
  lines.push(goalLine);
  return lines.join('\n');
}
...
export function registerPairCommand(ctx) {
  ctx.effect(() => ctx.commands.register({
    ...
    input: {
      ...
      images: true, // Legacy DSH command registries.
      attachments: true, // DSH 0.1.5 admits both images and files under this flag.
    },
    handler(invocation) {
        ...
        return {
          ...
          text: `Usage: /${PAIR_COMMAND} <goal> [--light] [--tdd=enforce|coach|off] [--style=traditional|strong|ping-pong]`
            + (attachments.length > 0
              ? ` — say what the ${attachments.length} attachment(s) are for; they stay in the composer, so just add the goal and submit again.`
              : ''),
        };
      }
      ...
      return {
        ...
        text: `Pair-programming activated — assembling a team for: ${goal}`
          + (attachments.length > 0 ? ` (${attachments.length} attachment(s) carried into the goal)` : ''),
      };
    },
