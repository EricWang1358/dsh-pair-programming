...
export function disclosureSummary(team, opts = {}) {
  const open = openDisclosures(team);
  if (open.length === 0) return 'none open';
  // `quoted: false` is for seats that must not read candidate evidence: the obligation and its kind
  // survive, the Driver's own words do not (#155/#156; the plan's item 5).
  const quoted = opts.quoted !== false;
  return open.map(item => `${item.ref} (${item.kind})${quoted ? ': ' + item.text : ''}`).join(' · ');
}
