...
function blocksCard(risk, taskId) {
  // An instrument defect is a defect in the MEASUREMENT, and the instrument is shared by
  // every card rather than owned by one: the card that happened to raise the ticket is an
  // accident of who was working that day. Reading `taskId` for it let the raising card keep
  // its block while every OTHER card's gate replayed the same broken judge and issued a
  // credential. Attribution is a statement about whose PRODUCT work is held up, so an
  // instrument ticket is treated as unattributed here — which is also why
  // blocksImplementation keeps its own product filter and never reaches this branch.
  if (scopeOf(risk) === 'instrument') return true;
