...
export function openRisk(protocol, { severity, scenario, trigger, suggestion, raisedBy, scope }) {
...
export function openBlockingRisks(protocol, severities = ['P0', 'P1']) {
  return protocol.risks.filter(r => (r.status === 'OPEN' || r.status === 'MITIGATED') && severities.includes(r.severity));
}
 ...
 */
export function blocksImplementation(protocol) {
  return (protocol.risks ?? []).filter(r => r.status === 'OPEN' && r.severity === 'P0' && scopeOf(r) === 'product');
}
 ...
 */
export function blocksVerification(protocol) {
  return (protocol.risks ?? []).filter(r => r.status === 'OPEN' && r.severity === 'P0');
}
