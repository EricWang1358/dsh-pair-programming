...
import { attentionSet, attentionLines } from './attention.js';
import { openDisclosures } from './disclosure.js';
import { lessonLine } from './lessons.js';
 ...
 * @param {{memberName?:string, role?:string, budget?:number, candidateVisible?:boolean}} [opts]
 */
export function boardDigest(team, opts = {}) {
  ...
  sections.push([
    ...
    // A recycled acceptance seat reads the same projection as a live one: its welcome digest is a
    // board read like any other (#155; the plan's item 7). Default true keeps every other caller.
    attentionLines(attentionSet(team, { candidateVisible: opts.candidateVisible !== false }), { limit: 5 }),
  ...
  if (lessons?.keep?.length || lessons?.try?.length) {
    sections.push([
      ...
      ...(lessons.try ?? []).slice(0, 3).map(t => `- try: ${trim(lessonLine(t), 220)}`),
    ].join('\n'));
  }
