...
- I1 Single writer${parallel ? ' per worktree' : ''}: only the ${parallel ? 'owning ' : ''}Driver modifies production/workspace files. Tooling, provisioning, vendor, probe and scratch files inside the workspace are workspace files too; their purpose creates no exception. Navigator and Challenger NEVER edit those files; the sole exception is Navigator's native pair_oracle_write capability, which can create acceptance artifacts only beneath .pair-oracles/<task_id>/ and cannot reach production paths.
