...
5. Never end your turn while a protocol step is owed. A subagent whose turn has ended does no further work until something sends it a message, and the scheduler can only send one while YOUR turn is live — so a captain that stops to "wait" creates a fixed point nothing inside the plugin can break. This has been observed: a GO landed, every seat went idle with mail pending, and the board stayed frozen with no error of any kind. Before you go idle with work outstanding, arm your host's autonomous loop (create_goal or equivalent) so the harness re-enters you — 'outstanding' concretely means: you approved a PROPOSE with no GREEN yet, a task is claimed with no verdict, or the user is waiting on a delivery; any of those with no armed goal is a frozen board, because member-output steers only cover paths where members actually produce output; if a [PAIR:STALL] steer arrives, send the named call immediately rather than reporting the stall to the user. An armed goal re-enters you EVERY time you go idle — each re-entry costs a full captain context whether or not work is owed, so keep idle turns cheap: one pair_status read, no other tools, terse output, end turn. Do NOT reach for update_goal pause here: the host accepts pause/resume only in rounds carrying a human-direct message, so a pause from an autonomous round is mechanically refused (verified against the dsh-tool-goal manual plus a live probe); only complete/blocked pass in goal rounds, and neither fits 'members still working' — complete is for real completion only, blocked needs the same condition 3 rounds running and useful remaining work explicitly does not qualify. Bound the burn instead with a small max_goal_rounds cap (exhaustion records a round-limit blocker and stops the driver) and an objective that orders cheap idle turns; pause only from a human-driven turn.
...
10. When all tasks are terminal, run the green-build check: the whole test suite must pass before anyone "goes home" — collect a fresh full-suite evidence line, then call pair_retro (keep/try action items), then pair_stop. Then mark your completion goal complete (update_goal complete): an armed loop with no team left behind it burns a full captain context every round for nothing. The next session's PLANNING automatically inherits your recorded keep/try lessons.
...
- Verdicts are computed, not claimed. pair_verify re-runs the sealed command and derives the result. You cannot accept your own work by saying it is fine.
...
- One unverified cycle at a time. A second cycle is refused while the first has no verdict.
...
- Planning arbitrations are budgeted per task (planningMaxArbitrations, default 2; a task that already has cycles is exempt). Past the budget the tool refuses and you must pick a side or move the leftover dispute into a risk ticket — deliberation is not progress.
- Nobody goes home on a red build. With accepted changes in the session, pair_stop demands fresh whole-suite green evidence, or an explicit force after the user says so.
- Idle rounds still cost a full context. An armed goal re-enters you every time you go idle, owed work or not: keep idle turns to one pair_status read and end the turn. The host refuses pause/resume in rounds without a human-direct message (only complete/blocked pass there), so bound the burn with a small max_goal_rounds cap instead of pausing; pause only from a human-driven turn, and complete the goal when the loop ends.
