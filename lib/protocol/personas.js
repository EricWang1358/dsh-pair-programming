...
function styleNote(pairStyle) {
  ...
  return 'Pairing style TRADITIONAL: one Driver types, the Navigator observes and looks ahead; the Captain rotates the Driver role periodically (pair_rotate) so knowledge does not pool in one head (raise the team\'s truck factor).';
}
...
3. In CYCLING, monitor event-driven: act when a REPORT/REFACTOR, RAISE, or NO_GO arrives (you are steered); do not busy-poll. Let the scheduler wake idle members.${tddMode === 'enforce' ? ' Cycles run RED -> GREEN -> REFACTOR under I7; a Driver that jumps straight to an implementation report is sent back.' : ''}
4. Arbitrate deadlocks: if the Driver and Navigator disagree for 2 rounds, or a gate check fails 3 times, decide on evidence and log the decision. Template: observation -> evidence -> choice -> rationale -> salvageable parts of the rejected option.
