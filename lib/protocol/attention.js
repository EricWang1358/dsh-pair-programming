...
export function debtKey(team, obligation) {
  ...
  const cycle = obligation.cycleId === undefined
    ? (team?.protocol?.cycles ?? []).filter(candidate => candidate.taskId === task?.id).at(-1)
  ...
  const state = {
    ...
    taskId: obligation.taskId, cycleId: obligation.cycleId, disclosureRef: obligation.disclosureRef,
...
export function attentionSet(team, opts = {}) {
  ...
  for (const stale of staleCredentials(team, opts)) {
    items.push({
      kind: 'stale-credential', who: team?.mode === 'solo' ? 'captain' : 'driver', tool: 'pair_gate_check', ref: stale.taskId,
