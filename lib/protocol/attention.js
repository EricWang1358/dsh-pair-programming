...
import { gateStateFingerprint, gateBindingDiff } from './gate.js';
...
export function staleCredentials(team, opts = {}) {
  ...
  for (const task of team?.tasks ?? []) {
    ...
    if (pass.binding?.gateStateSha !== gateStateFingerprint(team, task.id)) {
      // The refusal at pair_gate_check already names WHICH input moved, and — when the risk register
      // kept the same tickets — that a change to the digest's own definition reads exactly the same
      // from a stored digest (gate.js gateBindingDiff). This row is the other surface for one event,
      // so it reads from that one source instead of keeping a second, vaguer sentence: the panel's
      // to-do and the tool's refusal then agree, and neither sends the reader hunting a board write
      // that never happened (#167). Names only — the full sentences stay in the refusal, which is
      // not budgeted per step the way this line is.
      const moved = gateBindingDiff(team, task.id, pass.binding);
      const inputs = [...new Set(moved.map(line => line.split(':')[0]))];
      const definition = moved.some(line => line.includes("digest's own definition"));
      out.push({ taskId: task.id, why: 'the board moved after the gate ran'
        + (inputs.length ? ` (${inputs.join(', ')} changed)` : '')
        + (definition ? '; a change to the digest definition itself reads the same way — re-running the gate is the recovery' : '') });
      continue;
    }
...
export function attentionSet(team, opts = {}) {
  ...
  // Every open disclosure is ALREADY an owed pair_arbitrate on the frontier (obligation.js:272) and
  // a disclosure row below, so listing both asked the captain to rule twice on one ref, in two
  // different wordings — measured on the demo board, `cycle:c-t-3-1:beyond` rendered as an
  // obligation row and again as a disclosure row (#165). The disclosure row is the one kept: it
  // carries the declaration's own words, which is what the ruling is about. The frontier itself is
  // untouched — `obligation` and `obligations` still carry the entry, so blocking_cause, the
  // scheduler and pair_yield keep seeing the call they always saw.
  const disclosureRefs = new Set(openDisclosures(team).map(entry => entry.ref));
  for (const obligation of obligations) {
    if (obligation.tool === 'pair_arbitrate' && disclosureRefs.has(obligation.disclosureRef)) continue;
    items.push({
