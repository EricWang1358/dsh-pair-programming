...
export function attentionSet(team, opts = {}) {
  ...
  for (const risk of openBlockingRisks(team?.protocol ?? {}, ['P0', 'P1'])) {
    ...
    items.push({
      ...
      why: closable
        ? `${risk.severity} risk ${risk.id} is MITIGATED and still blocks a successful stop — close it with an executable artifact: pair_risk(action="close", risk_id="${risk.id}", evidence=…, closing_cmd=…, closing_exit=0, closing_paths=[…])`
