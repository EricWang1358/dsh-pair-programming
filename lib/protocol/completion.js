...
import { collaborationProblems } from './design.js';
...
export function completionReadiness(team, { greenRequired = true, greenEvidence = '', worktreeSha, ceLoads } = {}) {
  const failures = [];
  const untriaged = (team.product?.discoveries ?? []).filter(item => item.status === 'untriaged');
  if (untriaged.length) failures.push(`product discovery needs triage: ${untriaged.map(item => item.id).join(', ')} — use pair_backlog to schedule, defer or dismiss with rationale`);
  ...
  // The credential is the PASS, not the pointer to it. task.gatePassId is a
  // convenience field, and it can legitimately be absent: a failed gate used to
  // erase it (see tools/arbitrate.js), and a card completed before that rule
  // existed may simply lack it. Judging the pass keeps every check below exactly
  // as strict — the freshness checks run against the same pass — while a missing
  // pointer stops reading as "the epic cannot close". Measured: a fully verified
  // three-card run could not be recorded DONE for that missing field alone.
  const credentialFor = task => task.gatePassId ?? latestFor(task)?.id;
  const withoutPass = tasks.filter(task => task.status === 'completed' && latestFor(task) === undefined);
  if (withoutPass.length > 0) failures.push(`completed task(s) have no gate credential at all: ${withoutPass.map(task => task.id).join(', ')}`);
  const staleBoard = tasks.filter(task => {
    const pass = latestFor(task);
    return task.status === 'completed' && pass?.id === credentialFor(task)
  ...
  if (typeof worktreeSha === 'string') {
    const staleTree = tasks.filter(task => {
      const pass = latestFor(task);
      return task.status === 'completed' && pass?.id === credentialFor(task) && pass.binding?.worktreeSha !== worktreeSha;
    });
  ...
  failures.push(...collaborationProblems(team));
...
export function makeCompletionReceipt(team, greenEvidence, at = Date.now()) {
  ...
  const payload = {
    ...
    ...(team.product ? { productBacklog: team.product.discoveries.filter(item => item.status === 'deferred').map(({ id, observation, userValue, priority, rationale, scope }) => ({ id, observation, userValue, priority, rationale, scope })) } : {}),
