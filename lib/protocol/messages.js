...
/** Old writers appended derived NEXT prose after JSON. Never replay that debt. */
export function withoutLegacyNext(text) {
...
export function decodeMessage(text) {
  ...
  const rest = withoutLegacyNext(text.trimStart().slice(match[0].length)).trim();
