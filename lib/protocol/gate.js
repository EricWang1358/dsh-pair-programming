 ...
 *   risks      every P0/P1 on the board. Risks carry no task attribution (the
 *              register is team-wide), so a blocker found on ANOTHER card
 *              still invalidates this one: it is exactly the case where the
 *              verdict must be recomputed, and the gate cannot tell whose work
 *              it came from.
 ...
 *
 * What is deliberately NOT exempt: a P0/P1 whose status moves, because this
 * digest is compared for equality in eight modules and a fingerprint is
 * symmetric — "a new blocker invalidates but a closed one does not" is a
 * directional binding, not a fingerprint, and it would have to be built with
 * the consumers that do not exist yet.
 */
