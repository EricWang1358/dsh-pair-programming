...
function gateBoundCycle(cycle) {
  const binding = cycle?.verify?.binding;
  if (binding === undefined || (binding.breakdown === undefined && binding.breakdownIndex === undefined)) return cycle;
  const { breakdown: _breakdown, breakdownIndex: _breakdownIndex, ...kept } = binding;
  return { ...cycle, verify: { ...cycle.verify, binding: kept } };
}
