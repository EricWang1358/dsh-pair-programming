...
function itemDiff(input, before, after, payload) {
  ...
  // #131 took the risk lifecycle out of the bound ticket, and the next such change
  // to the digest's own definition moves EVERY credential once on upgrade. A
  // credential stores digests, not tickets, so "one ticket was reworded" and "the
  // definition moved under all of them" are indistinguishable from it — and the
  // line above sends the reader hunting a board write that never happened. Name
  // the alternative cause; it is the one refusal the reader cannot otherwise see.
  // The id set is the only thing that IS knowable here, so it is the condition.
  if (input === 'risks' && lines.length > 0
    && Object.keys(after ?? {}).every(key => before?.[key] !== undefined)
    && Object.keys(before ?? {}).every(key => after?.[key] !== undefined)) {
    lines.push(`${input}: no ticket was added or removed, so this can equally be the one-time move every credential takes when the digest's own definition changes (BOUND_RISK_FIELDS in this module) rather than a board write — the credential stores digests, so the two causes cannot be told apart from it; re-running the gate is the recovery.`);
  }
  return lines;
}
...
export function runGate(team, taskId, opts = {}) {
  ...
  //
  // A board that re-froze before this field existed keeps the old answer: the first seal is not
  // recoverable from a record that never stored it, and inferring one would read a late re-freeze
  // as an early seal. Those cards are no longer traps — a card-scoped ruling now permits cancelling
  // a started card (#128) — but this arm does not retroactively forgive them.
  if (enabled.has('oracle_precedes_impl') && oracle !== undefined) {
    ...
    const firstSeal = oracle.firstFrozenAt ?? oracle.frozenAt;
    checklist.oraclePrecedesImpl = currentOracleCycles.length === 0
      || (firstSeal ?? Number.POSITIVE_INFINITY) <= firstOpened;
    if (Number.isFinite(oracle.frozenAt) && Number.isFinite(oracle.firstFrozenAt) && oracle.frozenAt > oracle.firstFrozenAt) {
      ...
      checklist.oracleTightenedAfterImpl = { firstFrozenAt: oracle.firstFrozenAt, frozenAt: oracle.frozenAt, forks: oracle.forks ?? null };
    }
