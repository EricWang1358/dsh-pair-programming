...
export function goalLoopAdvice(teamId) {
  return [
    ...
    `Then monitor event-driven as usual: act on GREEN / RAISE / NO_GO / STALL, never busy-poll pair_status. Pause cannot disarm an idle round (host rule: pause/resume need a human message in-round, refused otherwise) — so an idle re-entry must be cheap: status read only, then end turn. Bound the burn with a small max_goal_rounds cap, not with pausing; complete the goal when the team stops.`,
  ].join(' ');
}
