...
export function notRunnableEvidence(run) {
  ...
  // The message above is only English. cmd.exe translates it, and on a non-UTF-8
  // console codepage the translation reaches us as mojibake, so the words cannot
  // be matched in any locale but one — measured on a zh-CN host, where a missing
  // program exits 1 with an unreadable tail and the typo was charged as a
  // product REJECT. The structure survives translation: the shell opens its
  // output by quoting the exact program name it was asked to run.
