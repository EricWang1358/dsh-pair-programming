...
export function advancePhase(protocol, to) {
  ...
  if (protocol.phase === 'RETRO' || protocol.phase === 'DONE') return false;
...
export function granularitySignal(cycles) {
  // Terminal means ACCEPTED, not step==='CLOSED'. No tool has ever moved a
  // cycle past VERIFIED — RISK_CHECKED and CLOSED were advertised by the legacy
  // chains and reached by nothing — so the old predicate matched zero cycles
  // and the enlarge branch could never fire. The controller was dead in every
  // measured run; this is the fix, not a behaviour change.
  // Shrink outranks enlarge. A cycle failing right now is stronger evidence
  // than three that went well before it, and the two rules differ in force:
  // shrink FORCES a smaller step, enlarge only SUGGESTS a larger one. The
  // reverse order let a clean streak mask an actively struggling cycle.
  ...
  const closed = cycles.filter(c => c.step === 'CLOSED' || c.verify?.verdict === 'accept');
