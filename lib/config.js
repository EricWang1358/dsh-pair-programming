...
export const Config = z.object({
  ...
  /** Experimental: start new teams with two isolated Drivers when a dedicated integration command is configured. */
  ...
  experimentalLeanPrompt: z.boolean().default(DEFAULTS.experimentalLeanPrompt),
  /** Whole-suite command run after each isolated dual-Driver integration. Empty keeps the experimental toggle inactive. */
  ...
  /** N5 liveness heartbeat period in ms: every tracked team is swept for stalled mailboxes. `0` disables (default `120000`). */
  heartbeatMs: z.number().default(DEFAULTS.heartbeatMs),
  /** Working-seat watchdog lease in ms since the member session's last durable event. Long productive turns renew it; `0` disables (default `600000`). */
  workingLeaseMs: z.number().default(DEFAULTS.workingLeaseMs),
  ...
  /** Member lifetime: `cycle` respawns members per Pair Cycle from a board digest (R2), `session` keeps one durable seat per role (default `session`). */
  memberLifetime: z.string().default(DEFAULTS.memberLifetime),
  /** Model for the acceptance-definition seat (navigator / solo SPEC): "provider/model" or a bare model id; empty = the captain's route (default empty). */
  navigatorModel: z.string().default(DEFAULTS.navigatorModel),
  /** Reasoning effort for that seat: empty = the captain route's effort, `default` = the target model's adapter default, otherwise an adapter-published effort id (default empty). */
  ...
  /** Bounded resumes granted to one seat whose turn was truncated at the output-token ceiling while the same call stayed owed (default `2`; `0` never resumes and escalates on the first truncation). Wall-clock liveness tuning like `heartbeatMs`, so it stays YAML-only. */
  maxTokenResumes: z.number().default(DEFAULTS.maxTokenResumes),
  /** Retro entries carried across sessions (default `3`). Each one is re-read by every future session and every recycled seat, so the cap forces ranking rather than accumulation. */
  maxCarriedLessons: z.number().default(DEFAULTS.maxCarriedLessons),
  /** Compound Engineering delivery lane: `off` (default, provider never registered) | `captain` | `advisory` | `full`. */
  ceLanes: z.string().default(DEFAULTS.ceLanes),
  /** Compound Engineering lane for sessions with no live pair team: `off` (default) | `gesture` | `curated` | `full`. */
  ceSoloLane: z.string().default(DEFAULTS.ceSoloLane),
  /** Explicit path to a local Compound Engineering checkout; empty auto-detects. */
  cePath: z.string().default(DEFAULTS.cePath),
  /** Opaque token whose change requests one fresh CE probe; the Settings card writes it. */
