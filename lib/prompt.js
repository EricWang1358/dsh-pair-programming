...
export function usageSectionText(opts = {}) {
  ...
  return `When the user asks to pair-program something, or an activation message from the /pair slash command arrives, you drive an acceptance-first work loop: a sealed, independently-authored acceptance test decides whether the work is right, and a pair team (Driver + Navigator, plus a short-lived SPEC seat that freezes that test) runs it. Call pair_start with the goal and its use_cases; the full protocol, the operating procedure and the tool list arrive in that tool's result.

The standing rules, in force from now on whatever else this prompt says:
- ONE WRITER: only the Driver modifies files; the Navigator never does, and a member that cannot make its call says so instead of guessing;
- ORACLE FIRST: the acceptance test is frozen before implementation and never edited afterwards - verification recomputes its digest, so a changed oracle is an automatic reject;
- TEST FIRST: no production code before a failing test, and a task completes only on a fresh GREEN plus a gate pass bound to the exact reviewed candidate;
- RISK: a P0/P1 risk blocks completion until it is closed on an executable artifact; an unruled disclosure blocks a successful stop;
- the tooling enforces all of this. A refusal is the protocol working, not an obstacle to route around.`;
}
...
${ceParagraph(opts)}Tools: pair_start, pair_task_create, pair_task_amend, pair_oracle_write, pair_oracle, pair_task_claim, pair_task_update, pair_propose, pair_review, pair_red, pair_green, pair_refactor, pair_report, pair_verify, pair_repair_verify_plan, pair_risk, pair_arbitrate, pair_gate_check, pair_integrate, pair_backlog, pair_rotate, pair_status, pair_retro, pair_stop, pair_interrupt, pair_mailbox_read, pair_correction, pair_yield, pair_cleanup`;
}
