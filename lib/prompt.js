...
5. pair_verify(stage="checkpoint") closes a small intermediate cycle by executing its predeclared verify_plan while keeping the full oracle as the final contract. pair_verify(stage="final") recomputes the seal and re-runs that full command. You cannot assert either result; a moved seal is an automatic REJECT. On final ACCEPT you must also record beyond_request and preexisting_at_risk — a re-run proves the requested behaviour and is blind to behaviour nobody requested, which is where a measured regression lived (a comma fix silently rewrote an existing list/tuple contract while the oracle and all 18 regression tests stayed green).
...
Tools: pair_start, pair_task_create, pair_task_amend, pair_oracle_write, pair_oracle, pair_task_claim, pair_task_update, pair_propose, pair_review, pair_red, pair_green, pair_refactor, pair_report, pair_verify, pair_risk, pair_arbitrate, pair_gate_check, pair_rotate, pair_status, pair_retro, pair_stop, pair_interrupt`;
}
