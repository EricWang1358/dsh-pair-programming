...
export function installPairScheduler(ctx, config, hosts = {}) {
    ...
    async kickMember(workspace, teamId, memberName, suppliedCaptain, signal) {
      ...
      await serializeMember(queueKey, async () => {
        ...
        if (unread.length > 0) {
          ...
          const redelivery = await deliverToMember(ctx, captain, member.id, fallbackMailboxPrompt(live, collapsed[0]), signal);
  ...
  const heartbeat = createHeartbeat({
    ...
    run: async ({ workspace, teamId }, signal) => {
      ...
      signal.throwIfAborted();
      if (await settledQuiet(stateRootOf(workspace, config), team)) { runtime.untrackTeam(workspace, teamId); return; }
      signal.throwIfAborted();
  ...
  ctx.on?.('agent/session-start', async ({ agent }) => {
    ...
    try {
      let scan = recoveryScans.get(workspace);
      if (scan === undefined) {
        ...
        recoveryScans.set(workspace, scan);
        if (recoveryScans.size > 256) recoveryScans.delete(recoveryScans.keys().next().value);
        void scan.catch(() => { if (recoveryScans.get(workspace) === scan) recoveryScans.delete(workspace); });
      }
  ...
  ctx.on?.('dispose', () => {
    ...
    recoveryScans.clear();
    lastNudge.clear(); memberActivity.clear(); parkedAttempts.clear();
  });
