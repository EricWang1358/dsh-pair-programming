...
import { prepareMailboxDelivery, finishMailboxDelivery, coalesceDelivery, deliveryKey,
  busyNotificationAllowed, releaseDeliverySeats, boundedMailboxPrompt, selectDeliveryMessages } from './mail-delivery.js';
import {
  beginTaskAttempt,
...
export function installPairScheduler(ctx, config, hosts = {}) {
  ...
  const runtime = {
    ...
    async kickCaptain(workspace, teamId, suppliedCaptain, signal) {
      ...
      if (disposed || signal.aborted) return;
      const stateRoot = stateRootOf(workspace, config), key = deliveryKey(stateRoot, teamId, 'captain');
      runtime.trackTeam(workspace, teamId);
      await coalesceDelivery(ctx, key, async () => {
        const team = await readTeam(stateRoot, teamId);
        if (team === undefined || isDispatchClosed(team.protocol.phase) || signal.aborted) return;
        const captain = liveCaptain(ctx, team.captainSessionId, suppliedCaptain);
        if (captain === undefined || captain.status !== 'idle') return;
        busyNotificationAllowed(ctx, key, captain.id, false);
        ...
        mailDiagnostics.set(`${teamId}\0captain`, { teamId, member: 'captain', selected: batch.count, bytes: batch.bytes,
          backlog: batch.backlog + (accepted ? 0 : batch.count), accepted, at: Date.now() });
    ...
    async kickMember(workspace, teamId, memberName, suppliedCaptain, signal) {
      ...
      const stateRoot = stateRootOf(workspace, config);
      runtime.trackTeam(workspace, teamId);
      const queueKey = deliveryKey(stateRoot, teamId, memberName);
      ...
      await coalesceDelivery(ctx, queueKey, async () => {
        ...
        busyNotificationAllowed(ctx, queueKey, member.id, false);
        ...
        if (batch !== undefined) {
          ...
          mailDiagnostics.set(declineKey, { teamId, member: memberName, selected: batch.count, bytes: batch.bytes,
            backlog: batch.backlog + (redelivery.ok ? 0 : batch.count), accepted: redelivery.ok, at: Date.now() });
  ...
  const syncMemberStatus = async (agent, status) => {
    ...
    if (status === 'idle') {
      releaseDeliverySeats(ctx, [agent.id]);
