 ...
 */
import { deliverToMember, installRetiredInboxGuard } from './members.js';
  ...
  const runtime = {
    ...
    async heartbeat() {
      for (const { workspace, teamId } of [...activeTeams.values()]) {
        ...
        if (team.protocol?.phase === 'DONE' || team.protocol?.phase === 'ABORTED') { activeTeams.delete(`${workspace}\0${teamId}`); continue; }
    ...
    async escalateIfStalled(workspace, teamId) {
      ...
      const text = [
        ...
        NL + attentionLines(attentionSet(fresh, { maxResumes: config.maxTokenResumes })),
    ...
    async kickMember(workspace, teamId, memberName, suppliedCaptain) {
      ...
      await serializeMember(queueKey, async () => {
        ...
        if (unread.length > 0) {
          ...
          const redelivery = await deliverToMember(ctx, captain, member.id, fallbackMailboxPrompt(live, collapsed[0]), new AbortController().signal);
          await withLock(teamLockKey(stateRoot, team.id), () => (redelivery.ok
          ...
          // carries it.
          if (!redelivery.ok) decline(`could not redeliver ${ids.length} queued message(s) — ${redelivery.reason}`);
          return;
        }
        ...
        if (owed !== undefined && owed.who === member.name) {
          ...
          if (wasTruncated(member)) {
            ...
            ].join(String.fromCharCode(10)), new AbortController().signal);
            if (resumed.ok) {
        ...
        const assigned = await deliverToMember(ctx, captain, ticket.memberId, assignmentPrompt(ticket, config.stateDir, team.id), new AbortController().signal);
