      ...
      await coalesceDelivery(ctx, queueKey, async () => {
        ...
        const ticket = await withLock(teamLockKey(stateRoot, team.id), async () => {
          ...
          const task = recoverOwned ? owned : owned === undefined
            ? (fresh.parallel ? fresh.tasks.find(candidate => candidate.status === 'pending' && candidate.handoffId === undefined && claimEligibility(fresh, currentMember, candidate) === undefined) : nextReadyTask(fresh.tasks, currentMember.name, currentMember.role))
            : undefined;
          if (task === undefined) {
