...
import { claimEligibility, prepareTaskWorkspace } from './parallel-tasks.js';
import { rankReadyTasks } from '../protocol/product.js';
import { recycleMember } from './recycle.js';
import { stallDiagnosis, stallEscalation, lastProgressAt } from '../protocol/stall.js';
import { nextObligation, obligationLine, blockingCause } from '../protocol/obligation.js';
...
function nextReadyTask(tasks, memberName, role) {
  const ready = rankReadyTasks(tasks, task => task.status === 'pending'
  ...
  return ready.find(t => t.assignee === memberName && (t.priority ?? 2) === (ready[0]?.priority ?? 2)) ?? ready[0];
}
...
/** Runs a pause removed from the sweep; a background track must not undo one. */
const pausedTeams = new Map();
/** True only while the heartbeat's own kick is on the stack (see the heartbeat run body). */
let inBackgroundSweep = false;
  ...
  const runtime = {
    ...
    trackTeam(workspace, teamId, { background = false } = {}) {
      if (disposed) return;
      const key = `${workspace}\0${teamId}`;
      // A pause outranks BACKGROUND compensation. untrackTeam removes a run, but a
      // heartbeat pass already in flight keeps the snapshot it took at its start and its
      // kickTeam re-tracks the run on the way out — which made the pause last only until
      // the end of that pass (reviewer probe on HEAD 9a0ca54: tracked 0 right after the
      // pause, 1 when the pass finished, and later sweeps keep scanning). Deliberate
      // re-engagement — a captain tool call, a start, a resume — passes no flag and clears
      // the pause; the automatic edges (a member idle turn, a peer mailbox recovery kick)
      // carry the flag, so "paused until the captain speaks" is what actually happens.
      if ((background || inBackgroundSweep) && pausedTeams.has(key)) return;
      pausedTeams.delete(key);
      activeTeams.set(key, { workspace, teamId });
    },
    ...
    untrackTeam(workspace, teamId) {
      ...
      const key = `${workspace}\0${teamId}`;
      activeTeams.delete(key);
      pausedTeams.set(key, Date.now());
    },
    /**
     * Whether a captain pause is holding this run off the sweep list. Read-only, so a
     * delivery can decline its own wake without touching the pause itself.
     */
    isPaused(workspace, teamId) {
      return pausedTeams.has(`${workspace}\0${teamId}`);
    },
    ...
    async escalateIfStalled(workspace, teamId) {
      ...
      const diagnosis = stallDiagnosis(fresh, {
        unread, obligation: owed, workingLeaseMs: config.workingLeaseMs,
        // Two live reports said "all four seats idle" and nothing else, which is
        // what sent the captain back to the call it had already failed to make.
        blockingCause: blockingCause(fresh, { obligation: owed, oracleFirst: config.oracleFirst }),
      });
    ...
    async kickTeam(workspace, teamId, suppliedCaptain, signal, { background = false } = {}) {
      if (disposed || signal?.aborted) return;
      const stateRoot = stateRootOf(workspace, config);
      const team = await readTeam(stateRoot, teamId);
      if (team === undefined || isDispatchClosed(team.protocol.phase)) return;
      // The flag says WHO asked for this kick. A member's tool call is not the captain
      // speaking, so it must not lift a pause the captain set (review #75).
      runtime.trackTeam(workspace, teamId, { background });
      ...
      const results = await Promise.allSettled([
        ...members.map(member => runtime.kickMember(workspace, teamId, member.name, captain, signal, { background })),
    ...
    async kickCaptain(workspace, teamId, suppliedCaptain, signal, { background = false } = {}) {
      ...
      runtime.trackTeam(workspace, teamId, { background });
      // defer: a captain wake that arrives during this flight must survive it
      // (M16' — the dropped wake edge); see coalesceDelivery.
    ...
    async kickMember(workspace, teamId, memberName, suppliedCaptain, signal, { background = false } = {}) {
      signal = signal === undefined ? lifetime.signal : AbortSignal.any([signal, lifetime.signal]);
      if (disposed || signal?.aborted) return;
      const stateRoot = stateRootOf(workspace, config);
      runtime.trackTeam(workspace, teamId, { background });
      ...
      // defer: this is a WAKE. Dropping it left a seat idle with pending=1 in
      // its mailbox and nothing left to re-read the board (M16').
      await coalesceWake(ctx, queueKey, async () => {
        if (disposed || signal?.aborted) return;
        ...
        const ticket = await withLock(teamLockKey(stateRoot, team.id), async () => {
          ...
          const task = recoverOwned ? owned : owned === undefined
            ? (fresh.parallel ? rankReadyTasks(fresh.tasks, candidate => candidate.status === 'pending' && candidate.handoffId === undefined && claimEligibility(fresh, currentMember, candidate) === undefined)[0] : nextReadyTask(fresh.tasks, currentMember.name, currentMember.role))
            : undefined;
          if (task === undefined) {
          ...
          await prepareTaskWorkspace(fresh, currentMember, task, config);
          const attemptId = fresh.parallel && task.attemptId ? task.attemptId : beginTaskAttempt(task, currentMember.name);
        ...
        await withLock(teamLockKey(stateRoot, team.id), async () => {
          ...
          if (fresh.parallel && task.workspace) return; // Preserve a dispatched workspace/attempt for retry, never silently reassign it.
          task.status = 'pending';
  ...
  const syncMemberStatus = async (agent, status) => {
    ...
    await withLock(teamLockKey(stateRoot, located.id), async () => {
      ...
      if (next === 'working') {
        ...
        // Work time adds the turn that just ended, once: a repeated idle edge finds the seat already idle.
        // A start left stale by a host restart counts at most one working lease past its last sampled activity.
        if (current.status === 'working' && Number.isFinite(current.activity?.startedAt)) {
          const started = current.activity.startedAt;
          const lease = config.workingLeaseMs > 0 ? config.workingLeaseMs : Infinity;
          const end = Math.min(now, Math.max(started, current.activity.lastActivityAt ?? started) + lease);
          current.workMs = (Number.isFinite(current.workMs) && current.workMs >= 0 ? current.workMs : 0) + Math.max(0, end - started);
        }
    ...
    if (status === 'idle') {
      ...
      // The idle edge is AUTOMATIC: a seat finishing its turn must not lift the captain
      // pause. Measured on the defect HEAD: this one line re-tracked a paused run, so the
      // pause lasted only until the next seat went idle, not until the captain spoke.
      await runtime.kickMember(workspace, located.id, member.name, undefined, undefined, { background: true });
    }
  };
  ...
  ctx.on?.('agent/error', ({ agent, error }) => {
    ...
    void (async () => {
      ...
      const request = seatModelRequest(config, fallbackRole, team);
      if (fallbackRole !== undefined && request.model !== undefined && isQuotaError(String(error))) {
        ...
        try {
          setNavRouteFallback('模型 ' + request.provider + '/' + request.model + ' 用量耗尽（'
            + String(error).slice(0, 160) + '）——' + fallbackRole + ' 席位已回退为队长模型与配置', team);
          markNavFallback();
        } catch (markError) {
  ...
  const heartbeat = createHeartbeat({
    ...
    run: async ({ workspace, teamId }, signal) => {
      ...
      signal.throwIfAborted();
      // Background compensation, marked as such: this kick belongs to a pass that may
      // have started BEFORE a pause, so anything it re-tracks must not undo that pause.
      // (A delivery or a tool call reaching kickTeam outside this window is deliberate and
      // still re-engages the run.) The mark covers exactly this await; a delivery landing
      // inside it would be treated as background and would not clear the pause - a window
      // the size of one kick, against a defect that used to last until the captain spoke.
      inBackgroundSweep = true;
      try { await runtime.kickTeam(workspace, teamId, undefined, signal); }
      finally { inBackgroundSweep = false; }
      signal.throwIfAborted();
  ...
  ctx.on?.('agent/session-start', async ({ agent }) => {
    ...
    try {
      ...
      for (const team of snapshot.teams) {
        if (team.captainSessionId !== agent.id || ['DONE', 'ABORTED'].includes(team.protocol.phase)) continue;
        // J2: the board on disk owns the checkout. When ANOTHER non-terminal team owns
        // it, this conversation keeps READ-ONLY visibility and no member is tracked —
        // tracking is what wakes them, and waking them is how two teams end up writing
        // one production workspace after a restart. The refusal is logged with the
        // occupant and the way out rather than thrown: a session starting up must not
        // fail because a board it did not know about is open elsewhere.
        const owner = await workspaceOwner(stateRootOf(workspace, config), team.id);
        if (owner !== undefined) {
          ctx.logger?.warn?.('pair-programming: ' + team.id + ' was not resumed here — ' + workspaceBusyError(owner).message);
          continue;
        }
        runtime.trackTeam(workspace, team.id);
      }
