...
export function reviewValue(team) {
  ...
  for (const c of cycles) {
    ...
    // A cycle can carry a live review decision AND a live verify verdict at the same time, so "the
    // current rejection" is a SET, not one entry — picking one counted the other as undated even
    // though the notes loop above displays it with a date (item 12). Each live verdict the record
    // does not already date is displayed there, so all of them are subtracted; a record whose own
    // timestamp is missing is not in `recorded` and is not live either, so it stays counted and is
    // reported as undated instead of disappearing from both lists.
    const liveVerdicts = [
      c.review?.verdict === 'no_go' ? { kind: 'no_go', at: c.review.at } : null,
      c.verify?.verdict === 'reject' ? { kind: 'reject', at: c.verify.at } : null,
    ].filter(Boolean);
    const displayedLive = liveVerdicts.filter(v => time(v.at) !== null && !recorded.some(p => p.kind === v.kind && p.at === v.at)).length;
    undatedPushbacks += Math.max(0, count(c.rejections) - recorded.length - displayedLive);
  }
