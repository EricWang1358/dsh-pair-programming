 ...
 * whose runtime data (data/*.json) is gitignored can therefore not be verified
 * there: its oracles report PREREQ and its suite reports an instrument failure,
 * which the integration reads as a failed candidate. Seed the canonical
 * workspace's ignored data files into the merged checkout so the merged tree is
 * a complete environment. Copies, never links: this worktree is force-removed
 * afterwards and a junction must never be traversed by that removal. Existing
 * (tracked or already-merged) files are never overwritten, and any failure here
 * is deliberately left to the acceptance oracles to report.
 */
function seedRuntimeData(workspace, worktree) {
  try {
    const source = join(workspace, 'data');
    if (!existsSync(source)) return;
    const target = join(worktree, 'data');
  ...
  } catch { /* best-effort seed; the acceptance oracles report a missing pool themselves */ }
}
...
export async function integrateCandidate(parallel, candidate, { verify, signal } = {}) {
  ...
  return withLock(`pair-integration:${fold(await rootOf(parallel.workspace))}`, async () => {
    ...
    try {
      ...
      signal?.throwIfAborted();
      seedRuntimeData(parallel.workspace, path);
