...
export async function createDriverWorktrees(workspace, stateRoot, teamId) {
  ...
  try {
    for (const name of ['driver', 'driver2']) {
      ...
      await git(workspace, ['worktree', 'add', '-b', slot.branch, slot.path, baseHead]);
...
export async function integrateCandidate(parallel, candidate, { verify, signal } = {}) {
  ...
  return withLock(`pair-integration:${fold(await rootOf(parallel.workspace))}`, async () => {
    ...
    try {
      ...
      await git(parallel.workspace, ['worktree', 'add', '--detach', path, previousHead]); created = true;
      signal?.throwIfAborted();
      await git(path, [...identity, 'merge', '--no-edit', '--no-ff', candidate.commit]);
      ...
      await git(parallel.workspace, ['merge', '--ff-only', '--no-edit', mergedHead]);
