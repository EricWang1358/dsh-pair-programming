 ...
 * into the child's creation context. Where it is not injected the property
 ...
 * literal on EVERY host, approval service or not, so the guard `if (policies)`
 * never tested host capability. On such a host the throw aborted every isolated
 * (dual-Driver) spawn before the child existed.
 *
 * `ctx.get(name)` is not the escape hatch it looks like: it reads the context's
 * own reflective store (cordis `_getImpl`) and stays undefined unless a provider
 * on THAT context is active, and during setup the child is still unpublished. So
 * on this host neither setup-time candidate can supply a session and the created
 * handle is the only carrier that can — which is what `policyState` exists to
 * keep from applying the same overrides twice.
 *
 * Copying policy must never be the reason a spawn fails: report it and let the
 * caller retry through the handle returned by `ctx.agents.create`, whose
