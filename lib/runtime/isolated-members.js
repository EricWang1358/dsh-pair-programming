...
import { registerWorkspace } from './workspace-context.js';
...
function runtime(ctx) {
  ...
  if (!value) {
    value = { handles: new Map(), closing: false };
...
function setupFor(parent, member, policies) {
  return childCtx => {
    if (policies) appendDelegatedPolicyOverrides(childCtx.agent.session, policies);
...
export async function spawnIsolatedMember(ctx, config, captain, team, member, persona, deny, signal) {
  ...
  const handle = await ctx.agents.create({
    ...
    signal, setup: setupFor(captain, member, policies),
  });
...
export async function deliverIsolatedMember(ctx, captain, childId, text, signal) {
  ...
  const team = await findTeamByCaptain(stateRootOf(captain.session.header.cwd, owned.config), captain.id);
