...
import { spawnIsolatedMember, deliverIsolatedMember, interruptIsolatedMember } from './isolated-members.js';
...
export function installRetiredInboxGuard(ctx, config) {
  ...
  ctx.on?.('agent/session-start', async ({ agent }) => {
    ...
    try {
      const ids = await readRetiredMemberIds(stateRootOf(cwd, { stateDir: config?.stateDir ?? '.pair-programming' }));
      if (ids.has(agent.id)) {
 ...
 */
/** Isolated contributors implement assigned work; they do not orchestrate
 * additional agents or author/review their own acceptance contract. Keep the
 * filesystem/shell, research and image inspection tools needed to do the job. */
export function isolatedDriverDenyList(knownTools) {
  if (!knownTools || knownTools.size === 0) throw new Error('isolated Driver needs a readable host tool registry');
  const protocol = new Set(['pair_status', 'pair_task_claim', 'pair_task_update', 'pair_propose', 'pair_red', 'pair_green',
    'pair_refactor', 'pair_report', 'pair_gate_check', 'pair_mailbox_read', 'pair_backlog', 'pair_risk']);
  const work = new Set(['read', 'write', 'edit', 'glob', 'grep', 'read_file', 'write_file', 'create_file', 'edit_file',
    'str_replace_editor', 'apply_patch', 'multiedit', 'multi_edit', 'bash', 'pwsh', 'powershell', 'shell', 'exec',
    'exec_command', 'read_image', 'view_image', 'skill', 'web_fetch', 'web_search', 'job_list', 'job_output', 'job_kill',
    'vision_html_screenshot', 'vision_screenshot', 'vision_present', 'vision_ocr', 'vision_describe']);
  return [...knownTools].filter(name => !protocol.has(name) && !work.has(name.toLowerCase()));
}
 ...
 *
 * KEYED BY THE RUN, NOT BY THE PROCESS. Measured live (v0.15.0 U1): one team's
 * acceptance seat died of quota and every OTHER team in the same host silently
 * resolved onto the captain's route, because this was one module-level variable.
 * That made it the seventh route-resolution path and the only one that leaked
 * across teams. The settings route test belongs to no run, so a call that passes
 * no team keeps the process-scope marker it always had; every production caller
 * passes the team, and a marker blocks exactly the scope it was recorded for.
 */
const navRouteFallbacks = new Map();

/** The run a route decision belongs to: artifactNamespace is minted per run. */
function routeScopeOf(team) {
  if (team === undefined || team === null) return undefined;
  const namespace = String(team.artifactNamespace ?? '').trim();
  return namespace !== '' ? namespace : String(team.id ?? '');
}
const fallbackKey = scope => `${scope ?? '*'}\0navigator`;

/** Process-wide view, for the settings card that mirrors the marker. */
export function navRouteFallbackActive() { return navRouteFallbacks.size > 0; }
/**
 * The process-wide readout, for a settings card. Now that a fallback belongs to a RUN,
 * several runs can be degraded at once — and returning the first one's reason would
 * report one run's state as the state of the process, which is the same conflation the
 * per-team key was introduced to end. One degraded run still reports its own reason
 * verbatim; more than one says how many, so a reader is never told a partial truth.
 */
/**
 * The fallback active for ONE run, or null. The process-wide readout above cannot answer
 * this, and seatModelRequest cannot either: it returns {} both when a run is on the
 * fallback and when it simply has no override, so a board that wants to SHOW the route
 * needs the question asked per run.
 */
export function navRouteFallbackFor(team) {
  return navRouteFallbacks.get(fallbackKey(routeScopeOf(team))) ?? null;
}
export function navRouteFallbackReason() {
  if (navRouteFallbacks.size === 0) return null;
  const first = navRouteFallbacks.values().next().value;
  if (navRouteFallbacks.size === 1) return first;
  return navRouteFallbacks.size + ' runs are on a fallback route; the first reports: ' + String(first);
}
/** Record a quota death for one run (or for the process when no team is given). */
export function setNavRouteFallback(detail, team) { navRouteFallbacks.set(fallbackKey(routeScopeOf(team)), String(detail ?? '')); }
/**
 * Clear the marker. Called with no arguments by the successful settings route
 * test — which carries no team, and which the recovery contract says restores
 * the seat — so it clears EVERY scope deliberately: a run whose premium route
 * answers again must stop being routed on the captain's model.
 */
export function clearNavRouteFallback() { navRouteFallbacks.clear(); }

export function seatModelRequest(config, role, team) {
  if (role !== 'navigator' && role !== 'spec') return {};
  // Sticky fallback wins over the composed override: the premium route is
  // known-dead until the user re-selects and a route test passes. The key is the
  // one this run recorded, so another run's quota death cannot re-route this seat.
  if (navRouteFallbacks.has(fallbackKey(routeScopeOf(team)))) return {};
...
export async function spawnMember(ctx, config, selections, llmSelection, captain, team, member, personaText, signal) {
  if (team.parallel && member.role === 'driver') {
    Object.assign(member, llmSelection);
    return spawnIsolatedMember(ctx, config, captain, team, member, personaText, isolatedDriverDenyList(hostToolNames(ctx)), signal);
  }
...
export async function deliverToMember(ctx, captain, childId, text, signal) {
  ...
  try {
    const isolated = await deliverIsolatedMember(ctx, captain, childId, text, signal);
...
export function interruptMember(ctx, captain, childId) {
  try {
    if (interruptIsolatedMember(ctx, childId)) return true;
