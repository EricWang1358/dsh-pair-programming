...
export function boardWriteDenial(team, agentId, workspace, name, args) {
  ...
  if (member.role === 'driver') {
    ...
    const cycle = (team.protocol.cycles ?? []).filter(item => item.taskId === task.id).at(-1);
    ...
    for (const target of targets) {
      const allowed = task.scope?.declared ? task.scope.writes : cycle.proposal?.files ?? [];
      const scopedPath = foldPath(relative(resolve(workspace), resolve(workspace, target)).replaceAll('\\', '/'));
      if (!allowed.some(value => { const root = foldPath(value.replaceAll('\\', '/')); return scopedPath === root || scopedPath.startsWith(`${root}/`); })) {
