...
import { isWriteCapability, hostToolNames } from './members.js';
 ...
 */
export function boardWriteDenial(team, agentId, workspace, name, args, { knownToolNames } = {}) {
  ...
  // #21: a name the host's GLOBAL registry does not carry cannot be classified by
  // this plugin, and for a member that must never mean "allow". The deny list is built
  // from the global view while restrict() validates a scope-aware set that also carries
  // ANCESTOR contributions (the agent-preset plane), so a write tool living only there
  // is neither enumerated nor classified here — both layers would pass it, and I1
  // would fail open on exactly the seat it exists to constrain. The SDK names the
  // failure itself: "Once presets moved them onto the agent plane they became an
  // ANCESTOR contribution, so a child's filter silently stopped constraining anything
  // it was given."
  //
  // Fail-closed, with the remedy in the message: a tool that belongs on a preset plane
  // and is genuinely read-only has to be visible to the registry this plugin can read,
  // because a guard that guesses is the bug it is closing. An unreadable registry
  // leaves the rule off rather than refusing every call — team formation already
  // refuses a seat whose write names cannot be enumerated (members.js, fail-loud).
  if (!isWriteCapability(name) && knownToolNames !== undefined && !knownToolNames.has(name)) {
    return 'pair-programming: refused ' + name + ' for ' + member.name + ' (role=' + member.role + ') on team "' + team.id
      + '": this plugin cannot classify that name, because the host registry it can read does not carry it (a tool contributed by an ancestor plane, such as an agent preset, is invisible to the global enumeration while restrict() would still accept it). '
      + 'It is refused rather than allowed because the alternative is an unclassifiable tool being a write capability on a seat that may not write. If it is genuinely read-only, make it visible to the global tool registry, or do the work through the Driver.';
  }
...
export function installBoardWriteGuard(ctx, config) {
  ...
  // Read once, at install: the registry is the host's, and re-reading it per call
  // would put a schema walk on the hot path of every member tool call.
  const knownToolNames = hostToolNames(ctx);
  ...
  return ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    // #21: a name the host registry does NOT carry is precisely the one this guard
    // cannot classify — so it must not be filtered out here. Letting it through early
    // made the member rule below unreachable for exactly the names it exists for: the
    // two sets are built from the global view, and an ancestor (agent-preset)
    // contribution appears in neither. The captain half ignores such a name, so the
    // only cost is one board read on a call the plugin would otherwise wave past.
    const unclassifiable = knownToolNames !== undefined && !knownToolNames.has(exec.name);
    if (!CAPTAIN_MUTATION_TOOLS.has(exec.name) && !isWriteCapability(exec.name) && !unclassifiable) return next();
    ...
    const reason = boardWriteDenial(entry.team, agent.id, workspace, exec.name, exec.arguments, { knownToolNames });
          ...
          for (;;) {
            try {
              ...
              if (actual !== root) {
                ...
                const resolvedReason = boardWriteDenial(entry.team, agent.id, root, exec.name, { path: resolvedTarget }, { knownToolNames });
