 ...
 */
import { readTeam, commitMemberReplacement } from '../state/store.js';
...
export async function recycleMember(ctx, config, runtime, stateRoot, teamId, memberName, opts = {}) {
  ...
  try {
    let selection = await resolveMemberLlmSelection(ctx, captain, seatModelRequest(config, member.role), opts.signal);
    ...
    const request = seatModelRequest(config, member.role);
    if (request.model !== undefined && isQuotaError(String(error))) {
      setNavRouteFallback('模型 ' + request.provider + '/' + request.model + ' 用量耗尽（'
        + String(error?.message ?? error).slice(0, 160) + '）——' + member.role + ' 席位已回退为队长模型与配置');
      try {
  ...
  try {
    if (!opts.signal?.aborted) committed = await commitMemberReplacement(stateRoot, teamId, member, replacement);
  } catch (error) {
