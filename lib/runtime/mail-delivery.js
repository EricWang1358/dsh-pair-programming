...
import { debtKey } from '../protocol/attention.js';
...
export function boundedMailboxPrompt(messages, team, recipient, config = {}, pending = messages.length) {
  ...
  // L2 at the seat, not only on the captain's board: the seat that owes the call is the one
  // that needs the skill covering it. Declared after owed, and used only by the branch that
  // actually delivers the instruction - the 'unchanged' branch stays silent on purpose.
  const momentNote = owed === undefined ? '' : (() => { const line = ceMomentLine(config.ceLanes ?? 'off', owed.tool); return line === undefined ? '' : ' ' + line; })();
  // #15: the footer carries the recipient's owed call, and it is charged against the
  // per-record allowance below — so a footer repeated delivery after delivery does not
  // merely take space, it takes the space REAL mail would have used (measured: 446 KB
  // of protocol traffic in one run, much of it the same obligation restated). The
  // obligation is identified by the same debt key the wake dedupe uses, so "the same
  // call, unmoved board" is one comparison and not a new notion of sameness.
  const debt = owed === undefined ? undefined : debtKey(team, owed);
  const told = debt === undefined ? undefined : (team.members ?? []).find(m => m.name === recipient)?.obligationDelivered?.debt;
  const restated = debt !== undefined && told === debt;
  const next = owed === undefined
    ...
    : restated
      ? metadata('[PAIR:NEXT] ' + owed.tool + ' — unchanged since your last delivery, so it is not repeated here; the full instruction is on the board (read pair_status first).', 2200)
      : metadata(obligationLine(owed, recipient) + momentNote, 2200);
  ...
  return { text, bytes: byteLength(text), count: messages.length, physicalCount, backlog: Math.max(0, pending - physicalCount), obligationDebt: debt, obligationRestated: restated };
}
...
export async function prepareMailboxDelivery(stateRoot, teamId, recipient, config, { memberId, captainId, signal, urgentOnly = false } = {}) {
  return withLock(teamLockKey(stateRoot, teamId), async () => {
    ...
    // Persist the memory only when the owed call actually changed: that is roughly once
    // per board move, not once per delivery, so the collapse does not buy its bytes back
    // with a team write on every message. It lives on the member record and is cleared
    // with the seat (store.js), because a replacement inherits nothing it was not told.
    if (prompt.obligationDebt !== undefined && prompt.obligationRestated !== true) {
      const target = team.members.find(member => member.name === recipient);
      if (target !== undefined) {
        target.obligationDelivered = { debt: prompt.obligationDebt, at: Date.now() };
