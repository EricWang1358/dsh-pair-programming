...
import { scopeConflicts, pathName as literalPath } from './worktrees.js';
...
export function claimEligibility(team, member, task) {
  if (task.ready === false) return 'TASK_NOT_READY: the product manager must finish discovery triage before a Driver claims this draft';
...
export async function prepareTaskWorkspace(team, member, task, config) {
  ...
  if (task.preparedAmendment && task.workspace && !team.protocol.cycles.some(cycle => cycle.taskId === task.id)) {
    if (task.preparedAmendment.owner !== member.name || task.workspace !== team.parallel.slots[member.name]?.path
      || await workspaceFingerprint(task.workspace, config) !== task.preparedAmendment.fingerprint) {
      throw new Error('TASK_WORKSPACE_STALE: amended preparation changed or lost its owner; preserve it for explicit recovery');
    }
    const slot = team.parallel.slots[member.name];
    if (await git(task.workspace, ['symbolic-ref', '--short', 'HEAD']) !== slot.branch
      || await git(task.workspace, ['rev-parse', 'HEAD']) !== task.baseHead) throw new Error('TASK_WORKSPACE_STALE: prepared branch changed; preserve it for recovery');
    const head = await git(team.parallel.workspace, ['rev-parse', 'HEAD']);
    // Fast-forward brings newly integrated dependencies into the preparation,
    // preserving local oracle files. Git refuses overwriting local changes;
    // no hard reset/clean can discard evidence on this recovery path.
    if (head !== task.baseHead) await git(task.workspace, ['-c', 'core.autocrlf=input', 'merge', '--ff-only', '--no-edit', head]);
    task.baseHead = head; slot.baseHead = head;
    delete task.preparedAmendment;
    return;
  }
  ...
  await git(path, ['-c', 'core.autocrlf=input', 'reset', '--hard', head]);
