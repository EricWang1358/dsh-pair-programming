...
import { disposeIsolatedMember } from './isolated-members.js';
...
export async function retireSpawnedMembers(ctx, captain, stateRoot, members) {
  ...
  for (const member of spawned) {
    ...
    await disposeIsolatedMember(ctx, member.id).catch(error => ctx.logger?.warn?.(`isolated Driver disposal failed: ${error.message}`));
  }
  let persisted = true;
